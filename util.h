...
// RAR lockstep diagnostic (defined in stack_printer.cpp): capture the current call stack / symbolize an address.
...
class RandomGen {
  ...
  void init(int seed);
  long long getDrawCount() const { return drawCount; } // RAR lockstep diagnostic: # of RNG draws since init
  // RAR lockstep: snapshot/restore the generator so the SIM's RNG stream can be isolated from RENDERING (FX,
  // animations, fire flicker all draw from this same global Random -- if not isolated, differing frame rates
  // between the two peers desync the simulation). The battle loop restores the sim state before each step and
  // saves it after, so render-time draws don't corrupt the deterministic sim stream.
  std::mt19937 saveGenerator() const { return generator; }
  void restoreGenerator(const std::mt19937& g) { generator = g; }
  static const int DRAW_TRACE_FRAMES = 12; // stack frames captured per draw when tracing
  void startDrawTrace() { drawTracing = true; drawTrace.clear(); }
  void stopDrawTrace() { drawTracing = false; }
  const vector<void*>& getDrawTrace() const { return drawTrace; } // flat, DRAW_TRACE_FRAMES per draw
  int get(int max);
  long long getLL();
  ...
  private:
  void captureDrawFrame(); // RAR lockstep diagnostic: append this draw's call stack to drawTrace
  std::mt19937 generator;
  long long drawCount = 0; // RAR lockstep diagnostic: incremented on every generator draw
  bool drawTracing = false;
  vector<void*> drawTrace;
