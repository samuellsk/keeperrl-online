...
class Level : public OwnedObject<Level> {
  ...
  void clearSectors(); // RAR: drop the (recomputable) pathfinding cache before serializing for upload
  struct EffectSet {
  ...
  HashMap<TribeId, unique_ptr<EffectsTable>> SERIAL(furnitureEffects);
