...
variant<AvatarInfo, AvatarMenuOption> getAvatarInfo(View* view,
  ...
  auto keeperCreatures = keeperCreatureInfos.transform([&](auto& elem) {
    return elem.second.creatureId.transform([&](auto& id) {
      auto ret = creatureFactory.fromId(id, elem.second.tribe.value_or(getPlayerTribeId(elem.second.tribeAlignment)));
  ...
  while (true) {
    ...
    // developerOnly is NOT the same as locked. A locked entry is deliberately still shown (greyed) so the player
    // can see what there is to unlock -- but developer content must not advertise its own existence, so it is
    // omitted from the list entirely. The mod stays active for everyone; only this menu hides it.
    auto keeperHidden = [&keeperCreatureInfos](int index) {
      return keeperCreatureInfos[index].second.developerOnly && !rarIsDeveloper();
    };
    // Never leave the default selection pointing at a hidden keeper -- it would be started without ever
    // appearing in the menu.
    while (keeperBase < keeperCreatureInfos.size() && keeperHidden(keeperBase))
      ++keeperBase;
    if (keeperBase >= keeperCreatureInfos.size())
      keeperBase = 0;
    for (int i : All(keeperCreatures)) {
      ...
      // Hidden only if EVERY variant under this base name is developer-only.
      auto baseHidden = [&] {
        for (int index : All(keeperCreatureInfos))
          if (keeperCreatureInfos[index].second.baseName == baseName && !keeperHidden(index))
            return false;
        return true;
      }();
      if (baseHidden)
    ...
    auto data = ScriptedUIDataElems::Record{{
      ...
      {"start_game", ScriptedUIDataElems::Callback{[&]{
        // ONLINE: identity is (account, keeper name), so this account may not already have a keeper with this
        // name -- creating one must NEVER silently take over the existing keeper's slot. Ask the server (the
        // source of truth) and send the player back to rename. A DIFFERENT account owning that name is fine.
        // Fail CLOSED if the server can't be asked: allowing it could overwrite a real keeper.
        if (rarEnabled())
          if (auto nm = getKeeperName()) {
            std::set<string> existing;
            if (!rarListKeepers(existing)) {
              view->presentTextCenter(TString(
                  "Couldn't reach the server to check your keeper names. Please try again."_s));
              return true; // ret unset -> back to the avatar menu
            }
            if (existing.count(rarComposeGameId(rarSessionLogin(), stripFilename(*nm))) > 0) {
              view->presentTextCenter(TString("You already have a keeper named \""_s + *nm +
                  "\".\nPlease choose a different name."_s));
              return true; // ret unset -> back to the avatar menu so the name can be changed
            }
          }
        PCreature keeper;
    ...
    // RAR online: the "Mods" button STAYS, but the server dictates the mod set, so it opens a READ-ONLY view
    // (the CHANGE_MOD handler routes online clicks to showServerMods -- no activate/add/remove for the client).
...
AvatarInfo getQuickGameAvatar(View* view, const KeeperCreatureInfo& myKeeper,
  ...
  AvatarInfo ret;
  ret.playerCreature = creatureFactory->fromId(myKeeper.creatureId[0], myKeeper.tribe.value_or(TribeId::getDarkKeeper()));
  if (!myKeeper.noLeader)
    ret.playerCreature->getName().setBare(TStringId("KEEPER_F"));
