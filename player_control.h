...
class PlayerControl : public CreatureView, public CollectiveControl, public EventListener<PlayerControl> {
  ...
  bool rarPassLeadership(Creature* victim); // RAR live PvP: hand team leadership on, with no dialog
  ...
  // RAR 4c teardown: drop every raw ref (message Positions, BattleSummary/attack/stun Creature*s) that
  // points into an invaded model that's about to be freed, so the invader's save stays clean.
  void scrubInvadedModelRefs(const Model* invaded);
  // RAR live PvP: same purpose, but WITHOUT inspecting any stored Position. scrubInvadedModelRefs must deref
  // each message's Position to test which model it belongs to -- and a message left over from an earlier battle
  // already points at a freed model, so the test itself crashes. This clears them all blindly instead.
  void rarClearStaleRefs();
  ...
  optional<TeamId> rarControlTeam(vector<Creature*>); // returns the team it created, so it can be cleaned up // RAR live PvP: take control of a whole team at once (invader's squad)
  void rarSetViewPos(Position); // RAR live PvP: point the overseer view at a position (e.g. the invaded base)
  long long rarGetViewLevelId() const; // RAR live PvP: the level this PC's build/click inputs resolve against
  Level* rarSwapCurrentLevel(Level*); // RAR live PvP: set currentLevel for a synced apply, returns the previous one
  ...
  public:
  // RAR pillage-on-conquest: called from the invasion-end flow so a conquered site can be pillaged on the
  ...
  public:
  // --reload_data (dev): rebuild the baked build menu from a freshly-reloaded ContentFactory.
  void reloadBuildMenu(const ContentFactory*, const KeeperCreatureInfo&);
  void reloadWorkshops(ContentFactory*, const KeeperCreatureInfo&);
  private:
