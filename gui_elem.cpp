...
static vector<string> breakText(Renderer& renderer, const string& text, int maxWidth, int size = Renderer::textSize(),
    char delim = ' ') {
  // Honor explicit '\n' even for short text (below the early-return); only single-line text can early-out.
  if (text.find('\n') == string::npos && renderer.getTextLength(text, size) <= maxWidth)
...
class ViewObjectConnectedGui : public GuiElem {
  public:
  ViewObjectConnectedGui(ViewId id, DirSet dirs, Vec2 sz, double sc, Color c)
      : id(id), dirs(dirs), size(sz), scale(sc), color(c) {}

  virtual void render(Renderer& renderer) override {
    renderer.drawViewObjectConnected(getBounds().topLeft(), id, dirs, true, scale, color);
  }

  virtual optional<int> getPreferredWidth() override { return size.x; }
  virtual optional<int> getPreferredHeight() override { return size.y; }
...
SGuiElem GuiFactory::viewObjectConnected(ViewId id, DirSet dirs, double scale, Color color) {
  return SGuiElem(new ViewObjectConnectedGui(id, dirs, Vec2(1, 1) * Renderer::nominalSize * scale, scale, color));
}

class ViewObjectExtraBorderGui : public GuiElem {
  public:
  ViewObjectExtraBorderGui(ViewId id, DirSet dirs, Vec2 sz, double sc, Color c)
      : id(id), dirs(dirs), size(sz), scale(sc), color(c) {}

  virtual void render(Renderer& renderer) override {
    renderer.drawExtraBorder(getBounds().topLeft(), id, dirs, scale, color);
  }
...
SGuiElem GuiFactory::viewObjectExtraBorder(ViewId id, DirSet dirs, double scale, Color color) {
  return SGuiElem(new ViewObjectExtraBorderGui(id, dirs, Vec2(1, 1) * Renderer::nominalSize * scale, scale, color));
}
