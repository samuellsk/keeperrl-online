...
void Workshops::refreshOptions(WorkshopArray options, const ContentFactory* factory) {
  for (auto& elem : options) {
    auto newOptions = elem.second.transform(
        [&](const auto& item){ return item.get(elem.first, factory); });
    if (auto type = getReferenceMaybe(types, elem.first))
      type->refreshOptions(std::move(newOptions));
    else
      types.insert(make_pair(elem.first, Type(newOptions)));
  }
  // Workshop types that exist in the save but not in the reloaded data are left as they are, so their
  // queues survive too.
}
...
void Workshops::Type::refreshOptions(vector<Item> newOptions) {
  // --reload_data: swap in the reloaded recipe list without throwing away what's already queued.
  // `indexInWorkshop` points into `options` -- checkDebtConsistency and "queue another one" both resolve
  // through it -- so every queued entry has to be re-pointed at its recipe's slot in the new list.
  for (auto& elem : queued) {
    // Match on name, which is a distinct TStringId per recipe even for near-identical variants. Cost is a
    // tiebreak in case a mod introduces two same-named recipes in one workshop.
    optional<int> newIndex;
    for (int i : All(newOptions))
      if (newOptions[i].name == elem.item.name) {
        if (newOptions[i].cost.id == elem.item.cost.id && newOptions[i].cost.value == elem.item.cost.value) {
          newIndex = i;
          break;
        }
        if (!newIndex)
    ...
    if (!newIndex) {
      // The recipe is gone from the data files. Carry it over so the queued item stays craftable and its
      // index stays valid; it just won't be offered for new orders.
      newOptions.push_back(elem.item);
      newIndex = newOptions.size() - 1;
    }
    elem.indexInWorkshop = *newIndex;
    // Take the refreshed cost/work values. Progress, payment and attached runes are left alone.
    elem.item = newOptions[*newIndex];
  }
  options = std::move(newOptions);
  // Rebuild rather than patch: a reload may have re-costed a queued recipe, and checkDebtConsistency
  // compares against options[].cost.
  debt.clear();
  for (auto& elem : queued)
    if (!elem.paid)
