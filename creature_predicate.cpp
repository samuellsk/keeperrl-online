...
// MinionTrait is COLLECTIVE membership state (not a creature attribute), so it's looked up on whichever
// collective the creature belongs to. A creature in no collective has no traits -> false.
static Collective* getCollective(const Creature*); // defined further down
static bool applyToCreature(MinionTrait t, const Creature* victim, const Creature* attacker) {
  if (auto col = getCollective(victim))
    return col->hasTrait(victim, t);
  return false;
}

static TString getName(MinionTrait t, const ContentFactory*) {
  return TString(string(EnumInfo<MinionTrait>::getString(t)));
}
...
// Total enemies this colony has killed, from Collective::getKills() -- the set of victims the colony recorded,
// which is what the kill counter in the UI is built from. Applied to a Position so it works anywhere.
static bool apply(const CreaturePredicates::TotalKills& s, Position pos, const Creature*) {
  if (auto game = pos.getGame())
    if (auto col = game->getPlayerCollective())
      return col->getKills().getSize() >= s.cnt;
  return false;
}

static TString getName(const CreaturePredicates::TotalKills& s, const ContentFactory*) {
  return TSentence("TOTAL_KILLS_AT_LEAST", TString(s.cnt));
}

// Minions this colony has lost. Summoned creatures and farm animals don't count -- see Collective::onMinionKilled.
static bool apply(const CreaturePredicates::TotalLosses& s, Position pos, const Creature*) {
  if (auto game = pos.getGame())
    if (auto col = game->getPlayerCollective())
      return col->getMinionsLost() >= s.cnt;
  return false;
}

static TString getName(const CreaturePredicates::TotalLosses& s, const ContentFactory*) {
  return TSentence("TOTAL_LOSSES_AT_LEAST", TString(s.cnt));
}
