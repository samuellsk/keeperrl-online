...
void ContentFactory::serialize(Archive& ar, const unsigned int version) {
  ...
  ar(immigrantsData, buildInfo, tribes, villains, gameIntros, keeperCreatures, technology);
  if (version == 0) {
...
optional<string> ContentFactory::readPlayerCreatures(const GameConfig* config, KeyVerifier* keyVerifier) {
  ...
  for (auto& keeperInfo : keeperCreatures) {
    ...
    // ALLY is deliberately NOT required: a keeper with no allies at all is legitimate (the NoMoreNecroAllies
    // mod, and any "everyone is my enemy" alignment). Nothing downstream needs one --
    // CampaignBuilder::placeVillainsInner() returns true immediately on an empty list, so such a keeper simply
    // gets no allies placed. On the shared RAR map an ally of another keeper then reads as an enemy, which is
    // exactly the point. MAIN/LESSER/MINOR stay required: without those the campaign map would be empty.
    for (auto type : {VillainType::MINOR, VillainType::LESSER, VillainType::MAIN})
...
optional<string> ContentFactory::readCampaignInfo(const GameConfig* config, KeyVerifier* keyVerifier) {
  ...
  // Upper bound was 20. That is a guard on the OFFLINE option sliders (setCountLimits builds their Range from
  // these), not on anything the generator can't handle -- and online the sliders aren't even shown, since
  // getCampaignOptions() returns only ENDLESS_ENEMIES/ENEMY_AGGRESSION when rarEnabled(). A 200x130 shared
  // world wants more villains than a single-player map, so the ceiling is raised rather than the counts
  // capped. Still bounded: a silly value should fail loudly here, not produce a 500-attempt placement stall.
  for (auto& l : {campaignInfo.maxAllies, campaignInfo.maxMainVillains, campaignInfo.maxLesserVillains})
    if (l < 1 || l > 64)
      return "Campaign villain limits must be between 1 and 64"_s;
...
optional<string> ContentFactory::readData(const GameConfig* config, const vector<string>& modNames) {
  ...
  map<PrimaryId<TribeId>, TribeDef> tribesTmp;
  if (auto error = config->readObject(tribesTmp, GameConfigId::TRIBES, &keyVerifier))
    return "Error reading tribes definition"_s + *error;
  tribes = convertKeys(tribesTmp);
...
void ContentFactory::merge(ContentFactory f) {
  ...
  mergeMap(std::move(f.tribes), tribes); // a mod can add tribes; earlier def wins on a duplicate key
