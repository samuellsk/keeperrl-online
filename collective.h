...
class Collective : public TaskCallback, public UniqueEntity<Collective>, public EventListener<Collective> {
  ...
  // Take ownership of the furniture on a WALL tile: register it as built and retribe it, but do NOT put the
  // position into territory. Territory is assumed walkable by tasks, pathing and population logic, so walls
  ...
  // How many of this colony's own minions have died. Summoned creatures and farm animals are NOT counted --
  // they are temporary/livestock, the same ones the morale code ignores. Kills are a set of the victims, but a
  // loss count has to be a plain counter: the dead are removed from the colony, so there is nothing left to size.
  int getMinionsLost() const;
  int getPoints() const;
...
// v1 added minionsLost. Without this the version stays 0 and the gated field is never written.
CEREAL_CLASS_VERSION(Collective, 1)
