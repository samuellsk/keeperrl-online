...
CreatureAction Creature::castSpell(const Spell* spell, Position target) const {
  ...
  return CreatureAction(this, [=] (Creature* c) {
    ...
    // RAR live PvP: announce the cast so the other player's copy of this creature shows the message and starts the
    // same cooldown. Only the ACT is announced -- the effect below stays local to the authoritative simulation.
    c->getGame()->addEvent(EventInfo::SpellCast{c, spell->getId()});
    c->spendTime();
...
//        INFO << getName().the() << " applying " << getPosition().getName();
        ...
        if (pos != getPosition() && getPosition() == originalPos)
          self->addMovementInfo(movementInfo
              ...
              .setFX(furniture->getUsageFX(), furniture->getUsageFXColor()));
      });
  return CreatureAction();
}
...
void Creature::rarSetMovementInfo(MovementInfo info) {
  addMovementInfo(info);
}
...
void Creature::addCombatIntent(Creature* attacker, CombatIntentInfo::Type type) {
  if (attacker != this) {
    lastCombatIntent = CombatIntentInfo{type, attacker->getThis(), *getGlobalTime()};
...
optional<Creature::CombatIntentInfo> Creature::getLastCombatIntent() const {
  if (lastCombatIntent && lastCombatIntent->attacker && !lastCombatIntent->attacker->isDead() &&
      !LastingEffects::restrictedMovement(lastCombatIntent->attacker.get()))
