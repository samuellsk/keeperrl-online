...
class GuiFactory {
  ...
  // Draws a tile using its connection-aware sprite variant for a fixed neighbour DirSet (auto-tiled walls
  // etc.). Used by the layout-preview dungeon grid, which computes each cell's DirSet from its neighbours.
  SGuiElem viewObjectConnected(ViewId, DirSet, double scale = 1, Color = Color::WHITE);
  // Draws a tile's extra-border sprite (shaded edge where two tile types meet, e.g. floor next to mountain).
  SGuiElem viewObjectExtraBorder(ViewId, DirSet, double scale = 1, Color = Color::WHITE);
