...
static void handle(const Effect& effect, Position pos, Furniture* furniture) {
  // Run a COPY. `effect` is a reference INTO furniture->tickType, and a tick effect is allowed to destroy or
  // replace its own furniture -- Furniture::tick even says so ("this function can delete this"). ModifyFurniture
  // does *f = data, which overwrites the furniture in place and frees this very Effect while it is executing;
  // a Chain then keeps iterating a vector that no longer exists and applies a freed Effect. Copying detaches
  // the running effect from the object it may destroy, so any self-modifying tick chain is safe.
  Effect running = effect;
  auto creator = furniture->getCreator();   // read before the effect can invalidate the furniture
  running.apply(pos, creator);
}
