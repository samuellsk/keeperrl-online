...
void Level::serialize(Archive& ar, const unsigned int version) {
  ...
  // furnitureEffects is now keyed by TribeId (was an EnumMap over the old tribe enum). On save this moves the
  // tables out into tmp; the loop below moves them back, so the live map is preserved either way.
  for (auto& elem : furnitureEffects)
    if (!!elem.second)
      tmp.push_back(make_pair(elem.first, std::move(elem.second)));
  ar(tmp);
  for (auto& elem : tmp)
    if (elem.second) {
      auto& table = furnitureEffects[elem.first];
      CHECK(!table);
...
void Level::forEachEffect(Vec2 pos, TribeId tribe, Fun f) {
  if (auto& effects = furnitureEffects[tribe])
    for (auto effect : effects->operator[](pos).friendly)
      f(effect);
  for (auto& elem : furnitureEffects)
    if (elem.first != tribe && elem.second)
      for (auto effect : elem.second->operator[](pos).hostile)
        f(effect);
}
...
void Level::clearSectors() {
  sectors.clear(); // lazily rebuilt by getSectors() on demand
}
