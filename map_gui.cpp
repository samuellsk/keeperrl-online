...
bool MapGui::onLeftClick(Vec2 v) {
  if (v.inRectangle(getBounds())) {
    mouseHeldPos = v;
    // A click on the map is only turned into a TILE_CLICK on release, and only if lastMouseMove is set -- but it
    // is set solely by actual mouse MOTION and is cleared by onMouseGone() whenever a movement KEY is pressed.
    // So "press a move key, then click without jiggling the mouse" produced NO click at all. Pressing the button
    // is itself proof the mouse is being used, so record the position here.
    lastMouseMove = v;
  ...
  if (dir.length8() == 1) {
    if (state >= 0.5 && state < 1.0) {
      ...
      if (movementInfo.fx && gid && furnitureUsageFX.getOrElse(*gid, -1) < movementInfo.moveCounter) {
        ...
        auto fxInfo = getFXInfo(*movementInfo.fx);
        if (movementInfo.fxColor)
          fxInfo.color = *movementInfo.fxColor;   // content override from furniture.txt usageFXColor
        addAnimation(FXSpawnInfo(fxInfo, pos + dir));
      }
    }
  ...
  if (spriteMode && tile.hasSpriteCoord()) {
    ...
    if (object.weaponViewId && tile.weaponOrigin && !object.hasModifier(ViewObject::Modifier::STUNNED)) {
      ...
      if (weaponTile.weaponOrigin) {
        ...
        bool creatureFlipped = object.hasModifier(ViewObject::Modifier::FLIPX);
        // RIGHT-HANDED minion: its hand (weaponOrigin) is in the RIGHT half of its tile. The art is drawn
        // left-handed, so a weapon attached there tilts INTO the body and hides behind it. Mirror the weapon so
        // it tilts outward instead. "Right half" = x past the midline: >=13 @24px, >=16 @30px == 2*x > width.
        bool rightHanded = 2 * tile.weaponOrigin->x > coord[0].size.x;
        bool weaponFlipped = creatureFlipped != rightHanded; // XOR: facing-flip and hand-flip combine
        auto orientation = Renderer::SpriteOrientation(false, weaponFlipped);
        // Align the weapon's grip to the minion's hand, mirroring each coordinate only if ITS sprite is flipped.
        int weaponGripX = weaponFlipped ? (weaponCoord[0].size.x - weaponTile.weaponOrigin->x) : weaponTile.weaponOrigin->x;
        int handX = creatureFlipped ? (coord[0].size.x - tile.weaponOrigin->x) : tile.weaponOrigin->x;
        auto offset = Vec2(weaponGripX, weaponTile.weaponOrigin->y) - Vec2(handX, tile.weaponOrigin->y);
