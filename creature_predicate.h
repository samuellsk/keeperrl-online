...
// Whole-colony totals, so a script can ask "has the player killed 30 things yet" directly instead of chaining
// flags to count. Both read the PLAYER's collective, whoever the predicate is applied to.
struct TotalKills {
  int SERIAL(cnt);
  SERIALIZE_ALL(cnt)
};
...
#define CREATURE_PREDICATE_LIST\
  ...
  X(IsDeepWater, 47)\
  X(AnyFurniture, 48)\
  X(MinionTrait, 49)  X(TotalKills, 50)  X(TotalLosses, 51)
