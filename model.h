...
class Model : public OwnedObject<Model> {
  ...
  // RAR live PvP: stop this model spawning its own attack waves. The invader holds a COPY of the defender's base;
  // if it keeps spawning locally it invents creatures the defender never had, which desyncs the battle.
  void rarDisableSpawning();
  ...
  int scrubForExport(); // RAR: drop non-player collectives + dead-creature log before model-only export (avoids dangling refs)
