...
void RandomGen::captureDrawFrame() {
  void* fr[DRAW_TRACE_FRAMES] = {};
  int n = rarCaptureStack(fr, DRAW_TRACE_FRAMES);
  for (int i = 0; i < DRAW_TRACE_FRAMES; ++i)
    drawTrace.push_back(i < n ? fr[i] : nullptr);
}
...
long long RandomGen::getLL() {
  ++drawCount; if (drawTracing) captureDrawFrame();
