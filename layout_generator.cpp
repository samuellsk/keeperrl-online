...
// Set only by MainLoop::previewLayout (LayoutErrorsThrowScope): see layout_canvas.h.
bool layoutErrorsThrow = false;
...
static Vec2 chooseSize(optional<Vec2> size, optional<Vec2> minSize, optional<Vec2> maxSize, RandomGen& r) {
  return size.value_or_f(
      [&]{
        LAYOUT_CHECK(!!minSize, "Layout generator: minSize is missing");
        LAYOUT_CHECK(!!maxSize, "Layout generator: maxSize is missing");
        LAYOUT_CHECK(minSize->x < maxSize->x, "Layout generator: minSize.x must be < maxSize.x");
        LAYOUT_CHECK(minSize->y < maxSize->y, "Layout generator: minSize.y must be < maxSize.y");
...
bool make(const LayoutGenerators::Place& g, LayoutCanvas c, RandomGen& r) {
  ...
  for (auto& generator : getGenerators()) {
    auto generate = [&] {
      LAYOUT_CHECK(generator.size || (generator.minSize && generator.maxSize),
          "Layout generator: Place needs either size, or both minSize and maxSize");
      ...
      for (int iter : Range(numTries)) {
        ...
        LAYOUT_CHECK(c.area.contains(genArea), "Layout generator does not fit in the area. "_s
            + toString(c.area) + " and " + toString(genArea));
