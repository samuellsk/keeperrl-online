...
void Model::prepareForRetirement() {
  ...
  for (auto& l : levels) {
    l->clearSectors(); // RAR: drop the (recomputable) pathfinding cache -- it's the bulk of a played
                       // dungeon's size (176MB->~7MB raw for an invaded writeback); rebuilt on load.
...
int Model::scrubForExport() {
  // RAR model-only export: a deeply-played base holds references to objects that live OUTSIDE the bare model,
  // which deserialize as DANGLING pointers when the model is re-homed into a fresh game (crashing the import
  // non-deterministically). Two culprits on complex bases: (1) non-PLAYER collectives -- cleared/alive z-level
  // + above-ground factions whose CollectiveControl references game-level state; (2) the dead-creature log,
  // whose entries reference the collectives/game. The reconstructed keeper needs neither -- only the player's
  // own faction + the live levels. Drop them. Living creatures stay on their levels (enemies become wild).
  // Never touch the PLAYER collective. Run on the model BEFORE serializing it model-only.
  int removed = 0;
  vector<PCollective> kept;
  for (auto& col : collectives) {
    if (col->getVillainType() != VillainType::PLAYER)
      ++removed;
    else
      kept.push_back(std::move(col));
  }
  collectives = std::move(kept);
  deadCreatures.clear();
  return removed;
}
