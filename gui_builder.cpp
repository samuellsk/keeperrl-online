...
void GuiBuilder::setGameSpeed(GameSpeed speed) {
  if (rarSpeedLocked)
    return;   // live PvP: both games must advance at the same rate, so speed changes are ignored
  gameSpeed = speed;
}
...
SGuiElem GuiBuilder::drawRightPlayerInfo(const PlayerInfo& info) {
  ...
  if (info.teamInfos.size() > 1) {
    ...
    // controlMode is OPTIONAL and is deliberately left unset during a live PvP battle (the squad is commanded by
    // clicking, so the Leader/Full toggle means nothing there). This dereferenced it unconditionally, so building
    // the panel walked an empty optional -- and everything queued after it, including the EXIT CONTROL button
    // below, never got added: there was no way to leave control mode at all.
    if (info.controlMode && !info.rarLivePvp) {
  ...
  // RAR live PvP only: drawn solely when PlayerInfo::rarAutoFight is set, so it never appears in a normal game.
  if (info.rarAutoFight) {
    auto autoCallback = getButtonCallback(UserInputId::RAR_TOGGLE_AUTO_FIGHT);
    vList.addElem(WL(stack,
        WL(standardButton,
            WL(label, TString(*info.rarAutoFight ? "Auto fight: ON"_s : "Auto fight: OFF"_s)),
            WL(button, autoCallback)),
        getTooltip({TString("Let this creature fight on its own while you are not giving it orders."_s)}, THIS_LINE)
    ));
    vList.addSpace(legendLineHeight / 2);
  }
...
// RAR: a former ally this keeper doesn't share is demoted to MINOR (an enemy) by reconcileVillains. It is
// PRESENTED as LESSER -- so it gets LESSER's yellow marker here, via the villain's display type. The site is
// still mechanically MINOR; only what the player sees changes.
static optional<Color> getSiteHighlightColor(const Campaign::SiteInfo& site) {
  if (auto villain = site.getVillain())
    return getHighlightColor(villain->getDisplayType());
  if (auto type = site.getVillainType())
...
void GuiBuilder::scrollWorldMap(int iconSize, Vec2 pos, Rectangle worldMapBounds, optional<Vec2> viewport) {
  auto maxSize = viewport.value_or(maxWorldMapSize) / iconSize;
  scrollAreaScrollPos = {
...
SGuiElem GuiBuilder::drawCampaignGrid(const Campaign& c, const vector<pair<Vec2, TString>>& claimedSites,
    optional<Vec2> initialPos, function<bool(Vec2)> selectable, function<void(Vec2)> selectCallback,
    const HashSet<Vec2>& conqueredKeepers, int baseTileSize, optional<Vec2> viewportOverride, bool connectTiles){
  HashMap<Vec2, TString> claimedMap; // other players' claimed sites (pos -> keeper name)
  for (auto& elem : claimedSites)
    claimedMap[elem.first] = elem.second;
  Vec2 viewport = viewportOverride.value_or(maxWorldMapSize);
  int iconScale = c.getMapZoom();
  int iconSize = baseTileSize * iconScale;
  int minimapScale = c.getMinimapZoom();
  auto& sites = c.getSites();
  // Keep the minimap a roughly FIXED on-screen size no matter how big the world is: the minimap draws the
  // whole world at minimapScale px/tile, so a bigger world would scale it up proportionally. Shrink the
  // per-tile scale (down to 1) so its total width stays near what a default ~100-tile-wide world gives.
  {
    int minimapTargetPx = 100 * c.getMinimapZoom();
    while (minimapScale > 1 && sites.getBounds().width() * minimapScale > minimapTargetPx)
      --minimapScale;
  }
  // RAR online: no fog of war -- the whole shared world is visible and any valid site
  // can be chosen. Offline keeps the normal influence-limited view.
  bool showAll = rarEnabled() || worldMapPreview;
  auto inView = [&](Vec2 p) { return showAll || c.isInInfluence(p); };
  // Auto-tiling (layout-preview dungeon): precompute each cell's set of connection-ids, then a helper that
  // derives a tile's 8-neighbour DirSet -- mirrors MapGui::getConnectionSet (match on connectionId) so walls
  // pick left-end/middle/T/corner variants instead of always rendering the fully-connected base sprite.
  const auto& tiles = renderer.getTileSet();
  Table<HashSet<ViewId>> connIds(sites.getBounds());
  if (connectTiles)
    for (Vec2 v : sites.getBounds())
      for (auto& id : sites[v].viewId)
        connIds[v].insert(tiles.getTile(id, true).getConnectionId().value_or(id));
  auto dirSetFor = [&](Vec2 tilePos, const ViewId& id) {
    DirSet ret;
    ViewId connId = tiles.getTile(id, true).getConnectionId().value_or(id);
    int cnt = 0;
    for (Vec2 dir : Vec2::directions8()) {
      Vec2 p = tilePos + dir;
      if (p.inRectangle(sites.getBounds()) && connIds[p].count(connId))
        ret.insert((Dir) cnt);
      ++cnt;
    }
    return ret;
  };
  // Extra borders (layout-preview dungeon): mirrors MapGui::renderExtraBorders. A tile with extra borders
  // (e.g. floor/grass) draws a shaded edge onto an adjacent tile of a type it borders against (e.g. mountain),
  // so solid furniture reads as a distinct region against the floor instead of an abrupt colour change.
  auto addExtraBorders = [&](Vec2 pos, vector<SGuiElem>& v) {
    HashSet<ViewId> sources; // neighbour viewIds (of pos) whose tile has extra borders
    for (Vec2 d : Vec2::directions4()) {
      Vec2 np = pos + d;
      if (np.inRectangle(sites.getBounds()))
        for (auto& nid : sites[np].viewId)
          if (tiles.getTile(nid, true).hasExtraBorders())
            sources.insert(nid);
    }
    for (auto& id : sources) {
      auto& tile = tiles.getTile(id, true);
      for (auto& borderId : tile.getExtraBorderIds())
        if (connIds[pos].count(borderId)) { // pos is a type `id` draws borders against
          DirSet dirs;
          for (Vec2 d : Vec2::directions4())
            if ((pos + d).inRectangle(sites.getBounds()) && connIds[pos + d].count(id))
              dirs.insert(d.getCardinalDir());
          v.push_back(WL(viewObjectExtraBorder, id, dirs, iconScale));
          break;
        }
    }
  };
  ...
  if (initialPos && campaignScrollCentered != initialPos) {
    scrollWorldMap(iconSize, Vec2(initialPos->x, initialPos->y), sites.getBounds(), viewport);
    campaignScrollCentered = initialPos;
  }
  for (int y : yRange) {
    ...
    for (int x : xRange) {
      auto pos = Vec2(x, y);
      // A cell can have no viewId (empty tile in a layout-preview dungeon) -> don't index an empty list.
      auto color = (sites[x][y].viewId.empty() ? Color::BLACK
          : renderer.getTileSet().getColor(sites[x][y].viewId.back())).transparency(150);
      if (auto type = sites[x][y].getVillainType())
        if (!rarIsVillainDead(x, y)) // world-map sync: don't highlight a server-defeated villain on the minimap
          if (auto c = getSiteHighlightColor(sites[x][y]))
            color = *c;
      if (!inView(pos))
        color = Color(0, 0, 0);
      minimapColumns.addElem(WL(stack,
          WL(button, [=, bounds = sites.getBounds()] { scrollWorldMap(iconSize, pos, bounds, viewport); }, true),
          WL(rectangle, color)));
    }
    ...
    for (int x : xRange) {
      ...
      if (inView(pos))
        for (auto& id : sites[x][y].viewId) {
          ...
          else if (connectTiles)
            v.push_back(WL(viewObjectConnected, id, dirSetFor(pos, id), iconScale));
          else
            v.push_back(WL(viewObject, id, iconScale));
        }
      if (connectTiles && inView(pos))
        addExtraBorders(pos, v); // shaded edges where solid furniture (mountain) meets floor/grass
    ...
    for (int x : xRange) {
      ...
      if (inView(Vec2(x, y))) {
        if (auto id = sites[x][y].getDwellingViewId())
          if (!c.isDefeated(Vec2(x, y)) && !rarIsVillainDead(x, y)) { // hide server-defeated villains (world-map sync)
            elem.push_back(WL(asciiBackground, id->front()));
            elem.push_back(WL(viewObject, *id, iconScale));
            labelPlacer.setOccupied(pos);
          }
        if (claimedMap.count(pos)) { // another player's claimed site
          elem.push_back(WL(asciiBackground, ViewId("map_dungeon1")));
          elem.push_back(WL(viewObject, ViewId("map_dungeon1"), iconScale));
          if (conqueredKeepers.count(pos)) // leader slain, in removal grace -> overlay the defeated marker
  ...
  for (int y : yRange) {
    auto columns = WL(getListBuilder, iconSize);
    for (int x : xRange)
      columns.addElem(inView(Vec2(x, y)) ? WL(empty) : translateHighlight(WL(viewObject, ViewId("map_fow"), iconScale)));
    ...
    for (int x : xRange) {
      ...
      if (inView(pos)) {
        if (!!sites[x][y].getVillainType() && !c.isDefeated(Vec2(x, y)))
          if (auto color = getSiteHighlightColor(sites[pos]))
        ...
        if (auto desc = sites[x][y].getDwellerName())
          if (!c.isDefeated(pos) && getSiteHighlightColor(sites[pos])) {
        ...
        if (claimedMap.count(pos)) { // label another player's claimed site with their keeper name
          auto& nm = claimedMap.at(pos);
          auto width = renderer.getTextLength(gui.translate(nm), 12, FontId::MAP_FONT);
          elem.push_back(WL(translate,
              WL(labelUnicode, nm, Color::YELLOW, 12, FontId::MAP_FONT),
              labelPlacer.getLabelPosition(pos, width), Vec2(width + 6, 18), GuiFactory::TranslateCorner::CENTER));
        }
      }
  ...
  int margin = 8;
  if (*mapContent->getPreferredWidth() > viewport.x || *mapContent->getPreferredHeight() > viewport.y)
        ...
        WL(alignment, GuiFactory::Alignment::TOP_RIGHT, WL(stack,
            ...
            WL(renderInBounds, WL(translate,
                ...
                WL(alignment, GuiFactory::Alignment::TOP_LEFT, WL(rectangle, Color::TRANSPARENT, Color::WHITE),
                    viewport * minimapScale / iconSize))))));
  return WL(preferredSize, viewport + Vec2(margin, margin) * 2, WL(stack, WL(rectangle, Color::BLACK),
    WL(miniBorder2),
...
SGuiElem GuiBuilder::drawWorldmap(Semaphore& sem, const Campaign& campaign, Vec2 current, bool preview) {
  worldMapPreview = preview; // offline map-generator: show the whole map, ignoring travel influence
  auto lines = WL(getListBuilder, getStandardLineHeight());
  ...
  // RAR online: overlay every OTHER keeper's base on the in-game world map. One server fetch per map open
  // (blocking modal). Skip only the tile we are standing on -- that is THIS keeper, already drawn from the
  // campaign's own keeper dweller. Filtering by login instead hid the account's OTHER keepers entirely: they
  // aren't in our campaign's dwellers, so nothing drew them. Same account or not, they are separate rivals.
  vector<pair<Vec2, TString>> claimedSites;
  HashSet<Vec2> conqueredKeepers; // slain bases still in their removal grace -> defeated sprite
  if (rarEnabled())
    for (auto& claim : rarGetClaims())
      if (Vec2(claim.x, claim.y) != campaign.getPlayerPos()) {
        Vec2 p(claim.x, claim.y);
        claimedSites.push_back(make_pair(p, TString(claim.name)));
        if (claim.conquered)
          conqueredKeepers.insert(p);
      }
  lines.addElemAuto(WL(centerHoriz, drawCampaignGrid(campaign, claimedSites, current, nullptr, nullptr,
      conqueredKeepers)));
  lines.addSpace(legendLineHeight / 2);
...
SGuiElem GuiBuilder::drawLayoutPreviewMenu(SyncQueue<View::LayoutPreviewAction>& queue, const Campaign* campaign,
      const View::LayoutPreviewInfo& info) {
  using Action = View::LayoutPreviewAction;
  using Id = View::LayoutPreviewActionId;
  // Dynamic scaling: size the whole preview to the app window, with the map viewport taking whatever is
  // left after the fixed control panel + margins.
  Vec2 winSize = renderer.getSize();
  int panelW = 240;
  int windowW = max(900, winSize.x - 40);
  int windowH = max(620, winSize.y - 40);
  Vec2 viewport(max(400, windowW - panelW - 90), max(320, windowH - 130));
  SGuiElem mapElem;
  if (campaign) {
    worldMapPreview = true; // show the whole generated map, ignoring travel influence
    campaignGridPointer = none;
    // Dungeon tiles are native furniture/wall sprites (24 or 30px), not the 8px world-map tiles -- render
    // them in 24px cells so they sit 1:1 instead of overlapping 3x (the "cramped" look).
    int baseTileSize = info.worldMap ? 8 : 24;
    mapElem = drawCampaignGrid(*campaign, {}, campaign->getSites().getBounds().middle(), nullptr, nullptr,
        {}, baseTileSize, viewport, !info.worldMap /*connectTiles: auto-tile dungeon walls*/);
    // Mouse-wheel zoom (like the in-game map). Topmost in the stack so it captures the wheel before the
    // scroll-area would pan; each notch re-runs the menu loop with an adjusted render zoom.
    mapElem = WL(stack, std::move(mapElem),
        WL(mouseWheel, [&queue] (bool up) { queue.push(Action{up ? Id::ZOOM_IN : Id::ZOOM_OUT, ""}); }));
  } else // generation failed at this size -> placeholder + hint instead of a map
    mapElem = WL(preferredSize, viewport, WL(stack, WL(rectangle, Color::BLACK),
        WL(margins, WL(centerHoriz, WL(label, TString("Couldn't generate this layout at this size -- try another size."_s))), 0, viewport.y / 2 - 30, 0, 0)));
  // Right-side control panel: type, size, and the scrollable layout list.
  auto right = WL(getListBuilder, getStandardLineHeight());
  right.addElem(WL(label, TString("Layout: "_s + info.current)));
  right.addSpace(8);
  right.addElem(WL(buttonLabel, TString(info.worldMap ? "Type: World map"_s : "Type: Dungeon"_s),
      [&queue] { queue.push(Action{Id::TOGGLE_TYPE, ""}); }));
  right.addSpace(8);
  right.addElem(WL(buttonLabel, TString("Width  (X): "_s + toString(info.sizeX)),
      [&queue] { queue.push(Action{Id::EDIT_X, ""}); }));
  right.addSpace(4);
  right.addElem(WL(buttonLabel, TString("Height (Y): "_s + toString(info.sizeY)),
      [&queue] { queue.push(Action{Id::EDIT_Y, ""}); }));
  right.addSpace(8);
  // The seed is what makes a good roll reproducible: note it here, then hand it to --rar_gen_world to build
  // the same terrain on the server. Clickable so a known seed can be typed straight in.
  right.addElem(WL(buttonLabel, TString("Seed: "_s + toString(info.seed)),
      [&queue] { queue.push(Action{Id::EDIT_SEED, ""}); }));
  right.addElem(WL(buttonLabel, TString("Reroll"_s), [&queue] { queue.push(Action{Id::REROLL, ""}); }));
  right.addSpace(10);
  right.addElem(WL(label, TString("Layouts:"_s)));
  auto listB = WL(getListBuilder, legendLineHeight);
  for (auto& name : info.layouts)
    listB.addElem(WL(buttonLabel, TString((name == info.current ? "> "_s : "   "_s) + name),
        [&queue, name] { queue.push(Action{Id::SELECT_LAYOUT, name}); }));
  if (info.worldMap || info.mappings.empty())
    right.addMiddleElem(WL(scrollable, listB.buildVerticalList(), &layoutPreviewScroll));
  else {
    // Dungeon: split the list area -- layouts on top (fixed), the mapping picker fills the rest below. The
    // mapping is the layout_mapping.txt entry that turns tokens (e.g. "rock") into furniture; pick to re-render.
    int layoutsH = max(120, (windowH - 360) * 3 / 5);
    right.addElem(WL(scrollable, listB.buildVerticalList(), &layoutPreviewScroll), layoutsH);
    right.addSpace(8);
    right.addElem(WL(label, TString("Mapping:"_s)));
    auto mapB = WL(getListBuilder, legendLineHeight);
    for (auto& name : info.mappings)
      mapB.addElem(WL(buttonLabel, TString((name == info.mapping ? "> "_s : "   "_s) + name),
          [&queue, name] { queue.push(Action{Id::SELECT_MAPPING, name}); }));
    right.addMiddleElem(WL(scrollable, mapB.buildVerticalList(), &layoutMappingScroll));
  }
  // Compose: map on the left, control panel on the right.
  auto body = WL(getListBuilder)
      .addElemAuto(std::move(mapElem))
      .addSpace(20)
      .addElemAuto(WL(setWidth, 240, right.buildVerticalList()))
      .buildHorizontalList();
  auto lines = WL(getListBuilder, getStandardLineHeight());
  lines.addElem(WL(centerHoriz, WL(label, TString("Layout preview  --  right-drag to pan, mouse wheel to zoom"_s))));
  lines.addMiddleElem(std::move(body));
  lines.addSpace(12);
  lines.addBackElem(WL(centerHoriz, WL(buttonLabel, TStringId("CLOSE_BUTTON"),
      [&queue] { queue.push(Action{Id::CLOSE, ""}); })));
  return WL(preferredSize, windowW, windowH, WL(window, WL(margins, lines.buildVerticalList(), 15),
      [&queue] { queue.push(Action{Id::CLOSE, ""}); }));
}

SGuiElem GuiBuilder::drawChooseSiteMenu(SyncQueue<optional<Vec2>>& queue, const TString& message,
    const Campaign& campaign, Vec2 initialPos, const vector<pair<Vec2, TString>>& invadeable) {
  auto lines = WL(getListBuilder, getStandardLineHeight());
  lines.addElem(WL(centerHoriz, WL(label, message)));
  campaignGridPointer = initialPos;
  // RAR online: rival keeper bases (invadeable) show as markers and are selectable even though
  // they're outside travel influence -- picking one launches an on-demand invasion.
  auto isInvadeable = [&invadeable](Vec2 pos) {
    for (auto& e : invadeable) if (e.first == pos) return true;
    return false;
  };
  // RAR online: villains are downloaded + invaded on demand, so they must be selectable ANYWHERE on the
  // shared map -- not just within travel influence (that gate is a single-player progression mechanic).
  // Any villain the roster still lists is selectable -- including a defeated-in-grace corpse, so the player
  // can revisit it to loot the aftermath. Once the server purges it (grace elapsed) reconcile drops the
  // dweller and it stops being pickable here.
  auto isVillainSite = [&campaign](Vec2 pos) {
    return rarEnabled() && !!campaign.getSites()[pos].getVillain();
  };
  auto canPick = [&campaign, isInvadeable, isVillainSite](Vec2 pos) {
    return campaign.canTravelTo(pos) || isInvadeable(pos) || isVillainSite(pos);
  };
  // Overlay the defeated marker on conquered rival bases (slain, in removal grace). One fetch per open.
  HashSet<Vec2> conqueredKeepers;
  if (rarEnabled())
    for (auto& claim : rarGetClaims())
      if (claim.conquered)
        conqueredKeepers.insert(Vec2(claim.x, claim.y));
  lines.addElemAuto(WL(centerHoriz, drawCampaignGrid(campaign, invadeable, initialPos, canPick, nullptr,
      conqueredKeepers)));
  lines.addSpace(legendLineHeight / 2);
  auto confirmCallback = [&, canPick] {
    if (canPick(*campaignGridPointer))
  ...
  lines.addElem(WL(centerHoriz, WL(getListBuilder)
      .addElemAuto(WL(conditional, WL(stack,
          ...
          [&, canPick] { return !!campaignGridPointer && canPick(*campaignGridPointer); }))
      .addSpace(15)
...
SGuiElem GuiBuilder::drawCampaignMenu(SyncQueue<CampaignAction>& queue, View::CampaignOptions campaignOptions,
  ...
  if (!rarEnabled()) // RAR online: the world map is server-fixed, no switching (it would reroll/desync)
  ...
  lines.addSpace(10);
  if (!rarEnabled()) // RAR online: no retired-dungeons option
  ...
  auto& claimedSites = campaignOptions.claimedSites;
  lines.addBackElemAuto(WL(centerHoriz, drawCampaignGrid(campaign, claimedSites, campaign.getOriginalPlayerPos(),
      [&campaign, &claimedSites](Vec2 pos) {
        for (auto& cs : claimedSites)
          if (cs.first == pos)
            return false; // can't start on a site claimed by another player
        return campaign.isGoodStartPos(pos);
      },
      [&queue](Vec2 pos) { queue.push({CampaignActionId::SET_POSITION, pos}); })));
  lines.addSpace(10);
  auto buttonList = WL(getListBuilder);
  buttonList.addElemAuto(WL(buttonLabelFocusable, TStringId("CONFIRM_BUTTON"), [&] { queue.push(CampaignActionId::CONFIRM); },
      [&menuState]{return menuState.index == CampaignMenuElems::Confirm{};}));
  if (!rarEnabled()) { // RAR online: the world is fixed by the server, so no reroll
    buttonList.addSpace(20);
    buttonList.addElemAuto(WL(buttonLabelFocusable, TStringId("REROLL_MAP_BUTTON"), [&] { queue.push(CampaignActionId::REROLL_MAP); },
        [&menuState]{return menuState.index == CampaignMenuElems::RollMap{};}));
  }
  buttonList.addSpace(20);
  buttonList.addElemAuto(WL(buttonLabelFocusable, TStringId("GO_BACK_BUTTON"), [&] { queue.push(CampaignActionId::CANCEL); },
      [&menuState]{return menuState.index == CampaignMenuElems::Back{};}));
  lines.addBackElem(WL(centerHoriz, buttonList.buildHorizontalList()));
...
SGuiElem GuiBuilder::drawChooseItemMenu(SyncQueue<optional<int>>& queue, const TString& title,
      const vector<View::ItemChoiceInfo>& items, const TString& cancelText) {
  auto lines = WL(getListBuilder, getStandardLineHeight());
  lines.addElem(WL(centerHoriz, WL(label, title)));
  const int zoom = 2;
  const int numColumns = 10;
  const int iconSize = Renderer::nominalSize * zoom + 10; // viewObject(zoom) + 5px margin each side
  // Width has to pay for EVERY inset between this window's edge and an icon, or the right-hand column slides
  // under the scrollbar and off the frame. The old "+ 40" missed two of them:
  const int gridMargin = 10;            // WL(margins, grid, 10) on the scrollable below
  const int scrollbarWidth = 22;        // gui_elem.cpp: scrollable() maybeMargin()s this off the RIGHT once
                                        // the grid is tall enough to scroll -- which it always is here
  const int windowFrameWidth = 2 * 20;  // GuiFactory::window wraps content in margins(..., 20, 35, 20, 30)
  const int windowWidth = numColumns * iconSize + 2 * gridMargin + scrollbarWidth + windowFrameWidth;
  auto grid = WL(getListBuilder, 20 * zoom);
  auto line = WL(getListBuilder, 20 * zoom);
  // Hover panel, same shape as the creature one in drawCreatureTooltip: name, then the item's stats and
  // description. A plain one-line tooltip can't show these, so this uses tooltip2 (an arbitrary elem) instead.
  const int tooltipWidth = 400;
  auto makeTooltip = [&] (const View::ItemChoiceInfo& item) {
    auto tipLines = WL(getListBuilder, legendLineHeight);
    tipLines.addElem(WL(label, item.name));
    for (auto& detail : item.details)
      // Wrapped: effect descriptions are full sentences and would otherwise run off the screen.
      tipLines.addElemAuto(WL(labelMultiLineWidth, detail, legendLineHeight * 2 / 3, tooltipWidth,
          Renderer::smallTextSize(), Color::LIGHT_GRAY));
    return WL(miniWindow, WL(margins, tipLines.buildVerticalList(), 15));
  };
  for (int i : All(items)) {
    // The tooltip MUST be stacked INSIDE the icon, not as a sibling of the mouseHighlight2 below.
    // GuiLayout::onMouseMove walks its children in REVERSE and, as soon as one returns true, sends
    // onMouseGone() to all the rest -- and mouseHighlight2 captures (returns true) whenever it is hovered.
    // So anything listed before it in the outer stack is visited later, gets onMouseGone(), and can never
    // render. Nesting it here means it is reached first, while hovered. This is also why the one-line name
    // tooltip that used to sit in that slot never appeared either. drawCreatureList nests it the same way.
    auto icon = WL(stack,
        WL(tooltip2, makeTooltip(items[i]), [](const Rectangle& r) { return r.bottomLeft(); }),
        WL(margins, WL(viewObject, items[i].viewId, zoom), 5, 5, 5, 5));
    line.addElemAuto(WL(stack, makeVec(
        WL(mouseHighlight2, WL(uiHighlight)),
        WL(button, [i, &queue] { queue.push(i); }),
        std::move(icon))));
    if (line.getLength() >= numColumns) {
      grid.addElemAuto(line.buildHorizontalList());
      grid.addSpace(10);
      line.clear();
    }
  }
  ...
  lines.addMiddleElem(WL(scrollable, WL(margins, grid.buildVerticalList(), 10)));
  lines.addSpace(15);
  if (!cancelText.empty())
    lines.addBackElem(WL(centerHoriz, WL(standardButton,
        gui.getKeybindingMap()->getGlyph(WL(label, cancelText), &gui, C_BUILDINGS_CANCEL, none),
        WL(button, [&queue] { queue.push(none);}))), legendLineHeight);
  int margin = 15;
  // Cap the height so the item grid SCROLLS instead of growing off-screen. Estimate the full height; if it
  // exceeds the screen, the window clamps to the screen and the middle scrollable takes over (with a scrollbar).
  int numRows = (int(items.size()) + numColumns - 1) / numColumns;
  int contentHeight = getStandardLineHeight() + numRows * (iconSize + 10) + legendLineHeight + 4 * margin + 20;
  int windowHeight = min(contentHeight, renderer.getSize().y - 40);
  return WL(preferredSize, 2 * margin + windowWidth, windowHeight,
      WL(window, WL(margins, lines.buildVerticalList(), margin), [&queue] { queue.push(none); }));
}
...
SGuiElem GuiBuilder::drawTextInput(SyncQueue<optional<string>>& queue, const TString& title, const string& value,
  ...
  auto lines = WL(getListBuilder, legendLineHeight);
  lines.addSpace(20);
  // Wrap the title so long prompts don't overflow the fixed-width dialog.
  lines.addElemAuto(WL(labelMultiLineWidth, title, legendLineHeight, 500));
