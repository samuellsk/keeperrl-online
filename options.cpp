...
const EnumMap<OptionId, Options::Value> defaults {
  ...
  {OptionId::LANGUAGE, 0},
  {OptionId::RELOAD_DATA, 0},
  {OptionId::NEVER_PAY_RANSOM, 0},
  {OptionId::DISABLE_ATTACK_WARNING, 0},
  {OptionId::RAR_SYNC_INFO, 0}
};
...
static TString getName(OptionId id) {
  switch (id) {
    ...
    case OptionId::RELOAD_DATA: return TString("Reload data files on load (dev)"_s);
    case OptionId::NEVER_PAY_RANSOM: return TString("Never pay ransom"_s);
    case OptionId::DISABLE_ATTACK_WARNING: return TString("Disable under attack warning (no pause)"_s);
    case OptionId::RAR_SYNC_INFO: return TString("Show world sync info on load (dev)"_s);
  }
};
...
const map<OptionSet, vector<OptionId>> optionSets {
  {OptionSet::GENERAL, {
      ...
      OptionId::NEVER_PAY_RANSOM,
      OptionId::DISABLE_ATTACK_WARNING,
      OptionId::SINGLE_THREAD,
      // UNLOCK_ALL is intentionally NOT listed: every keeper is unlocked from the start (Unlocks::isUnlocked),
      // so the toggle would be a no-op. The enum entry + default are kept so option indices stay stable.
#ifdef WINDOWS
      OptionId::DPI_AWARE,
#endif
      // ---- dev / diagnostic options: kept LAST so they don't clutter the normal settings ----
      OptionId::RELOAD_DATA,   // dev iteration; outside #ifndef RELEASE on purpose, so it shows in RELEASE
      OptionId::RAR_SYNC_INFO, // ditto: world-sync readout is only useful in a real (RELEASE) online session
#ifndef RELEASE
...
static bool isBoolean(OptionId id) {
  switch (id) {
    ...
    case OptionId::RELOAD_DATA:
    case OptionId::NEVER_PAY_RANSOM:
    case OptionId::DISABLE_ATTACK_WARNING:
    case OptionId::RAR_SYNC_INFO:
      return true;
    default:
      return false;
  }
...
void Options::readValues() {
  if (!values) {
    ...
    while (1) {
      // Was a fixed char[100] + istream::getline, which sets failbit on any longer line and then breaks out
      // of this loop -- so one long line (CURRENT_MOD2 grows with every mod enabled) silently reverted that
      // option AND every option below it to its default, which the next settings save then wrote back out.
      string buf;
