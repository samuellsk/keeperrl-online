...
// turn-based actions
  ...
  TOGGLE_CONTROL_MODE,
  EXIT_CONTROL_MODE,
  RAR_TOGGLE_AUTO_FIGHT,   // RAR live PvP only: controlled creature fights on its own when idle
  TOGGLE_TEAM_ORDER,
  TEAM_MEMBER_ACTION,
  CAST_SPELL,
  INVENTORY_ITEM,
...
struct BuildingClickInfo {
  ...
  int building;
  bool doubleClick;
  SERIALIZE_ALL(pos, building, doubleClick)
};
...
struct AIActionInfo {
  ...
  bool override;
  TString groupName;
  SERIALIZE_ALL(creature, switchTo, override, groupName)
};
...
struct WorkshopUpgradeInfo {
  ...
  int numItems;
  SERIALIZE_ALL(itemIndex, increases, numItems)
};
