...
void sortPositionsDeterministically(vector<Position>& ret) {
  std::sort(ret.begin(), ret.end(), [](const Position& a, const Position& b) {
    // Deref of getLevel() is safe here: these are currently-valid positions in a transient computation. levelId
    // is deterministic across processes; coord breaks ties within a level.
    auto la = a.getLevel() ? (long long) a.getLevel()->getUniqueId() : -1;
    auto lb = b.getLevel() ? (long long) b.getLevel()->getUniqueId() : -1;
    if (la != lb) return la < lb;
    if (a.getCoord().x != b.getCoord().x) return a.getCoord().x < b.getCoord().x;
    return a.getCoord().y < b.getCoord().y;
  });
}

vector<Position> positionsInDeterministicOrder(const PositionSet& positions) {
  vector<Position> ret(positions.begin(), positions.end());
  sortPositionsDeterministically(ret);
  return ret;
}

int Position::getHash() const {
  // NOTE: uses the raw Level* pointer VALUE (not level->getUniqueId()). This MUST stay pointer-based: Position is
  // a persistent, serialized key in HashMap<Position>/HashSet<Position> (task_map reversePositions, territory,
  // ...), and the hash must be stable for a stored key's whole lifetime. A deref-based hash (levelId) corrupts
  // those maps when a Level is destroyed and its address reused (stale key hashes to a new bucket) -> FATAL.
  // Determinism for lockstep is instead handled where it matters -- the AI SORTS position candidates before
  // Random.choose (see rarDeterministicOrder / the choose-site sorts) -- not by changing this hash.
...
void Position::getViewIndex(ViewIndex& index, const Creature* viewer) const {
  ...
  if (isValid()) {
    ...
    if (viewer) {
      if (auto& effects = level->furnitureEffects[viewer->getTribeId()])
        if (!effects->operator[](coord).friendly.empty())
          index.setHighlight(HighlightType::ALLIED_TOTEM);
      for (auto& elem : level->furnitureEffects)
        if (elem.first != viewer->getTribeId() && elem.second)
          if (!elem.second->operator[](coord).hostile.empty())
...
void Position::addFurnitureEffect(TribeId tribe, const FurnitureEffectInfo& effect) const {
  auto& effectsTable = level->furnitureEffects[tribe];
  if (!effectsTable)
...
bool Position::fireDamage(int amount, Creature* attacker) const {
  ...
  if (Creature* creature = getCreature())
    creature->takeDamage(Attack(attacker, Random.choose<AttackLevel>(), AttackType::HIT, amount,
        AttrType("FIRE_DAMAGE"), {}, TStringId("YOU_ARE_NOT_HARMED_BY_FIRE"), TStringId("IS_NOT_HARMED_BY_FIRE"), false));
...
bool Position::iceDamage(int amount, Creature* attacker) const {
  ...
  if (Creature* creature = getCreature())
    creature->takeDamage(Attack(attacker, Random.choose<AttackLevel>(), AttackType::HIT, amount,
        AttrType("COLD_DAMAGE"), {}, TStringId("YOU_ARE_NOT_HARMED_BY_COLD"), TStringId("IS_NOT_HARMED_BY_COLD")));
  for (Item* it : getItems())
    if (Random.chance(0.05 * amount))
      it->iceDamage(*this);
  return res;
...
bool Position::acidDamage(int amount, Creature* attacker) const {
  ...
  if (Creature* creature = getCreature())
    creature->takeDamage(Attack(attacker, Random.choose<AttackLevel>(), AttackType::HIT, amount,
        AttrType("ACID_DAMAGE"), {}, TStringId("YOU_ARE_NOT_HARMED_BY_ACID"), TStringId("IS_NOT_HARMED_BY_ACID")));
  /*for (Item* it : getItems())
