...
class Furniture {
  ...
  optional<Color> getUsageFXColor() const;
  /**
  ...
  // Optional colour override for usageFX. The variant's own colour is hardcoded in getFXInfo() (that is how
  // ENCHANTED_LABORATORY is just LABORATORY in purple); this lets content recolour one without a new variant.
  optional<Color> SERIAL(usageFXColor);
...
CEREAL_CLASS_VERSION(Furniture, 3)