...
void Renderer::drawViewObjectConnected(Vec2 pos, ViewId id, DirSet dirs, bool useSprite, double scale, Color color) {
  const Tile& tile = tileSet->getTile(id, useSprite);
  if (tile.hasSpriteCoord()) {
    optional<Color> colorVariant;
    if (!tile.animated)
      colorVariant = id.getColor();
    // Draw the background FILL first (e.g. the lava/water base), then the connection sprite (borders/shore),
    // then the CORNER sprites -- mirrors MapGui (getBackgroundCoord -> getSpriteCoord(dirs) -> getCornerCoords).
    // Corners are how solid furniture like MOUNTAIN gets its edges/shading against a neighbour (it uses
    // addCorner, not addConnection, so getSpriteCoord returns just the flat base sprite). Without corners the
    // background gave the fill, but solid tiles had no edge treatment where they met floor.
    drawTile(pos, tile.getBackgroundCoord(), Vec2(), color * tile.color, {}, colorVariant, scale);
    drawTile(pos, tile.getSpriteCoord(dirs), Vec2(), color * tile.color, {}, colorVariant, scale);
    if (tile.hasAnyCorners())
      for (auto& coord : tile.getCornerCoords(dirs))
        drawTile(pos, {coord}, Vec2(), color * tile.color, {}, none, scale);
  } else
    drawText(tile.symFont ? FontId::SYMBOL_FONT : FontId::TEXT_FONT, 20 * scale, color * tile.color,
        pos + Vec2(scale * nominalSize / 2, 0), tile.text, HOR);
}

void Renderer::drawExtraBorder(Vec2 pos, ViewId id, DirSet dirs, double scale, Color color) {
  // Draw a tile's extra-border sprite (the shaded edge a tile like floor/grass bleeds onto its neighbour where
  // it meets e.g. mountain). Mirrors MapGui::renderExtraBorders including the N/S and W/E straight-run splits
  // (there is no combined opposite-sides sprite, so draw each single-direction edge).
  const Tile& tile = tileSet->getTile(id, true);
  drawTile(pos, tile.getExtraBorderCoord(dirs), Vec2(), color * tile.color, {}, none, scale);
  if (dirs == DirSet{Dir::N, Dir::S}) {
    drawTile(pos, tile.getExtraBorderCoord({Dir::N}), Vec2(), color * tile.color, {}, none, scale);
    drawTile(pos, tile.getExtraBorderCoord({Dir::S}), Vec2(), color * tile.color, {}, none, scale);
  }
  if (dirs == DirSet{Dir::W, Dir::E}) {
    drawTile(pos, tile.getExtraBorderCoord({Dir::W}), Vec2(), color * tile.color, {}, none, scale);
    drawTile(pos, tile.getExtraBorderCoord({Dir::E}), Vec2(), color * tile.color, {}, none, scale);
  }
}
