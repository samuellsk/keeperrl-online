...
class Position {
  ...
  void removeFurniture(const Furniture*, PFurniture replace, Creature* destroyedBy = nullptr, bool secondPart = false) const;
...
// RAR lockstep determinism: unordered PositionSet/HashMap<Position> iterate in a process-dependent order
// (Position hashes by the Level* address). The AI builds movement/task candidate lists from these and then
// Random.choose()s / picks over them -- with an address-ordered list, two machines pick different tiles and
// desync. These impose a STABLE, cross-process order (by level unique id, then coord) on a TRANSIENT vector,
// which is safe (unlike ordering the persistent set itself, which would corrupt on level address reuse).
void sortPositionsDeterministically(vector<Position>&);
vector<Position> positionsInDeterministicOrder(const PositionSet&);
