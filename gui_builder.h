...
class GuiBuilder {
  ...
  struct Callbacks {
    ...
    function<void(const TString&)> info;
  };
  ...
  SGuiElem drawTextInput(SyncQueue<optional<string>>&, const TString& title, const string& value, int maxLength);
  ...
  SGuiElem drawChooseSiteMenu(SyncQueue<optional<Vec2>>&, const TString& message, const Campaign&, Vec2 initialPos,
      const vector<pair<Vec2, TString>>& invadeable = {});
  SGuiElem drawWorldmap(Semaphore&, const Campaign&, Vec2 current, bool preview = false);
  SGuiElem drawLayoutPreviewMenu(SyncQueue<View::LayoutPreviewAction>&, const Campaign*, const View::LayoutPreviewInfo&);
  SGuiElem drawChooseCreatureMenu(SyncQueue<optional<UniqueEntity<Creature>::Id>>&, const TString& title,
      const vector<PlayerInfo>&, const TString& cancelText);
  SGuiElem drawChooseItemMenu(SyncQueue<optional<int>>&, const TString& title,
      const vector<View::ItemChoiceInfo>&, const TString& cancelText);
  SGuiElem drawCreatureInfo(SyncQueue<bool>&, const TString& title, bool prompt, const vector<PlayerInfo>& creatures);
  SGuiElem drawCost(pair<ViewId, int>, Color = Color::WHITE);
  SGuiElem drawMinimapIcons(const GameInfo&);
  optional<int> getNumber(const TString& title, Vec2 position, Range, int initial);
  bool yesOrNo(const TString& question);
  ...
  void setActiveButton(int num, ViewId, optional<TString> activeGroup, optional<TutorialHighlight>, bool buildingSelected);
  void setActiveGroup(const TString& group, optional<TutorialHighlight>);
  optional<int> getActiveButton() const;
  GameSpeed getGameSpeed() const;
  void setGameSpeed(GameSpeed);
  // RAR live PvP: the two games must run at the SAME rate, so speed changes are ignored during a live battle
  // (pausing is already blocked for the same reason).
  void rarSetSpeedLocked(bool b) { rarSpeedLocked = b; }
  Rectangle getMenuPosition(int numElems);
  int getScrollPos(int index, int count);
  void setMapGui(shared_ptr<MapGui>);
  void clearHint();
  ~GuiBuilder();
  optional<int> chooseAtMouse(const vector<TString>& elems);
  ...
  // baseTileSize = source tile pixel size: 8 for the world map (small map_ tiles), 24 for the layout-preview
  // dungeon (native furniture/wall sprites, 24 or 30px). viewportOverride sizes the visible/scrollable area
  // (defaults to the fixed maxWorldMapSize when none) so the preview can scale to the app window.
  // connectTiles: auto-tile walls/rivers/etc. by computing each cell's neighbour DirSet (layout-preview
  // dungeon) instead of drawing every tile fully-connected. Off for the world map (its tiles don't connect).
  SGuiElem drawCampaignGrid(const Campaign&, const vector<pair<Vec2, TString>>& claimedSites,
      optional<Vec2> initialPos, function<bool(Vec2)> selectable, function<void(Vec2)> selectCallback,
      const HashSet<Vec2>& conqueredKeepers = {}, int baseTileSize = 8, optional<Vec2> viewportOverride = none,
      bool connectTiles = false);
  ...
  Options* options;
  Callbacks callbacks;
  SGuiElem getHintCallback(const vector<TString>&);
  SGuiElem getTooltip(const vector<TString>&, int id, milliseconds delay = milliseconds{700}, bool forceEnableTooltip = false);
  ...
  void scrollWorldMap(int iconSize, Vec2, Rectangle worldMapBounds, optional<Vec2> viewport = none);
  optional<int> itemIndex;
  optional<Vec2> campaignGridPointer;
  optional<Vec2> campaignScrollCentered; // RAR: last initialPos we auto-scrolled to, so we only center once
  bool worldMapPreview = false; // offline map-generator: show the whole map ignoring travel influence
  ScrollPosition layoutPreviewScroll; // offline map-generator: scrollbar of the layout list panel
  ScrollPosition layoutMappingScroll; // offline map-generator: scrollbar of the mapping list panel (dungeon)
  ...
  bool rarSpeedLocked = false; // RAR live PvP: ignore speed changes while a live battle is running
  TStringId getGameSpeedName(GameSpeed) const;
  TStringId getCurrentGameSpeedName() const;
  ...
  vector<TString> getItemHint(const ItemInfo&);
  ...
  vector<SGuiElem> getMultiLine(const TString& text, Color, int maxWidth, int fontSize);
  TString getPlayerTitle(PlayerInfo&);
  ...
  SGuiElem drawLyingItemsList(const TString& title, const ItemCounts&, int maxWidth);
  SGuiElem drawTickBox(shared_ptr<bool> value, const TString& title);
