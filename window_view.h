...
class WindowView: public View {
  ...
  virtual void rarSetInteractiveSpectator(bool) override;
  virtual int rarConsumeSpectatorKey() override;
  virtual void rarSetLivePvpInput(bool) override;
  virtual void rarSetSpeedLocked(bool) override;
  virtual void rarPumpEvents() override;
  virtual int rarConsumeClickMs() override;
  virtual bool rarHasPendingInput() override;
  virtual int rarGetActionCalls() override;
  ...
  virtual optional<Vec2> chooseSite(const TString& message, const Campaign&, Vec2 current,
      const vector<pair<Vec2, TString>>& invadeable = {}) override;
  virtual void presentWorldmap(const Campaign&, Vec2 current, bool preview) override;
  virtual LayoutPreviewAction previewLayoutMenu(const Campaign*, const LayoutPreviewInfo&) override;
  ...
  bool rarInteractiveSpectator = false; // RAR live PvP: SPECTATOR mode scrolls instead of exiting on any key
  int rarSpectatorKey = 0;              // RAR live PvP: last command key pressed in interactive-spectator mode
  bool rarLivePvpInput = false;         // RAR live PvP: queue keypresses instead of flushing/locking per turn
  int rarLastClickMs = 0;               // RAR live PvP diagnostic: when the last map click entered the queue
  int rarActionCalls = 0;               // RAR live PvP diagnostic: getAction() calls since that click
