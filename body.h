...
class Body {
  ...
  // RAR live PvP: mirror the authoritative side's health. There is no normal setter (damage needs a full
  // Attack), but the two games must agree on how wounded a creature is, so allow setting it directly.
  void rarSetHealth(double);
