...
RICH_ENUM(OptionId,
  ...
  DPI_AWARE,
  LANGUAGE,
  RELOAD_DATA, // dev: re-read data files into the game on each load (appended at the end to keep option indices stable)
  NEVER_PAY_RANSOM, // auto-decline attacker ransom demands (no prompt)
  DISABLE_ATTACK_WARNING, // under-attack notice becomes a non-blocking log message instead of a game-pausing popup
  RAR_SYNC_INFO // dev: on load, show for 2s whether the world map was downloaded or served from the local cache
);
