...
void TileSet::reload() {
  ...
  hasDefaultTiles = reloadDir(defaultDir, true);
...
void TileSet::loadTextures() {
  ...
  for (auto& elem : tileCoords)
    for (auto& coord : elem.second)
      CHECK(!!coord.texture);
  texturesTmp.clear();
  // Sprite mode requires the BASE tileset, not merely "something got loaded". Without data/images the game is
  // supposed to fall back to ASCII -- but a mod that ships its own sprites (orig16/24/30) still fills
  // tileCoords, which made this true and turned sprite rendering on with only the mod's tiles present. Every
  // vanilla sprite name then missed and byName() handed back tileCoords.begin() -- the mod's FIRST tile -- so
  // the whole game drew as one repeated image instead of ASCII.
  bool useTiles = hasDefaultTiles && !tileCoords.empty();
  if (useTiles)
    loadTiles();
