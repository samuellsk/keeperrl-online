  ...
  MoveInfo followTeamLeader() {
    ...
    if (auto team = getActiveTeam()) {
      if (!teams.hasTeamOrder(*team, TeamOrder::STAND_GROUND)) {
        ...
        if (creature != leader) {
          if (!creature->getPosition().isSameModel(leader->getPosition()))
            return NoMove;
...
// RAR live PvP AUTO FIGHT. Deliberately NOT collective(): that is the full minion AI, so between fights the
// creature took collective TASKS and wandered off -- across z-levels -- abandoning the spot the player had just
// moved it to. Here the creature only defends itself:
//   FighterStandGround = Fighter::getMove(false), so it attacks and casts at what comes into reach but never
//                        chases, which is what keeps a squad on the tile it was ordered to converge on;
//   Rest              = the fallback when there is nothing to fight (collective() falls back to MoveRandomly,
//                        which is the wandering);
//   pickItems=false   = don't break formation to walk over and grab loot.
// No ByCollective, no MoveRandomly, no GoldLust: nothing here can decide to go somewhere on its own.
MonsterAIFactory MonsterAIFactory::rarAutoFight(Collective* col) {
  return MonsterAIFactory([=](Creature* c) {
      return new MonsterAI(c, {
        new AvoidFire(c),
        new EffectsAI(c, col),
