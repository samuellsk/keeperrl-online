...
vector<pair<Position, FurnitureLayer>> MinionActivities::getAllPositions(const Collective* collective,
  ...
  // Deterministic order (built from unordered getBuiltPositions above) so applySquare's pick is lockstep-safe.
  std::sort(ret.begin(), ret.end(), [](const pair<Position, FurnitureLayer>& a, const pair<Position, FurnitureLayer>& b) {
    auto la = a.first.getLevel() ? (long long) a.first.getLevel()->getUniqueId() : -1;
    auto lb = b.first.getLevel() ? (long long) b.first.getLevel()->getUniqueId() : -1;
    if (la != lb) return la < lb;
    if (a.first.getCoord().x != b.first.getCoord().x) return a.first.getCoord().x < b.first.getCoord().x;
    if (a.first.getCoord().y != b.first.getCoord().y) return a.first.getCoord().y < b.first.getCoord().y;
...
static vector<Position> limitToIndoors(const PositionSet& v) {
  vector<Position> ret;
  ret.reserve(v.size());
  for (auto& pos : positionsInDeterministicOrder(v)) // deterministic order -> the derived task list is lockstep-safe
    if (pos.isCovered())
      ret.push_back(pos);
  return ret;
}
  ...
  switch (info.type) {
    case MinionActivityInfo::IDLE: {
        ...
        if (p->getLevel()->depth == 0)
          if (!myTerritory.empty() && collective->getGame()->getSunlightInfo().getState() == SunlightState::NIGHT) {
            ...
            return Task::chain(Task::transferTo(collective->getModel()),
                Task::stayIn(!indoors.empty() ? std::move(indoors) : positionsInDeterministicOrder(myTerritory)));
          }
      ...
      if (pigstyPos.count(c->getPosition()) && !myTerritory.empty()) {
        ...
        return Task::doneWhen(Task::goTo(Random.choose(positionsInDeterministicOrder(myTerritory))),
      ...
      if (!myTerritory.empty()) {
        ...
        return Task::chain(Task::transferTo(collective->getModel()), Task::stayIn(positionsInDeterministicOrder(myTerritory)));
    ...
    case MinionActivityInfo::ARCHERY: {
      ...
      if (!pos.empty())
        return Task::archeryRange(collective, tryInQuarters(positionsInDeterministicOrder(pos), collective, c));
      else
        return nullptr;
    }
    ...
    case MinionActivityInfo::EAT: {
      ...
      if (!hatchery.empty())
        return Task::eat(tryInQuarters(positionsInDeterministicOrder(hatchery), collective, c));
      break;
    }
