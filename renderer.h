...
class Renderer {
  ...
  // Like drawViewObject but picks the connection-aware sprite variant for the given neighbour DirSet
  // (walls/rivers/etc.) instead of always DirSet::fullSet(). Non-connecting tiles ignore the DirSet.
  void drawViewObjectConnected(Vec2 pos, ViewId, DirSet, bool useSprite, double scale = 1, Color = Color::WHITE);
  // Draws a tile's extra-border sprite for the given neighbour DirSet (shaded edge where two tile types meet,
  // e.g. floor next to mountain). Used by the layout-preview dungeon grid.
  void drawExtraBorder(Vec2 pos, ViewId, DirSet, double scale = 1, Color = Color::WHITE);
