...
#include "sound.h"
#include "game.h"
#include "rar_client.h" // RAR: rarEnabled() gate for pillage writeback re-arm
...
#include "campaign.h"
#include "enemy_info.h"        // RAR villages panel: leader viewId for world-map villains
...
PPlayerControl PlayerControl::create(Collective* col, vector<TString> introText, TribeAlignment alignment) {
  ...
  for (auto v : col->getKnownTiles().getAll()) {
    // Force the memory rebuild to actually write: addToMemory early-returns when a tile isn't flagged
    // needsMemoryUpdate, so without this a RECONSTRUCTED keeper (adopt -> fresh PlayerControl, no serialized
    // memory) starts with EMPTY visual memory -> the whole base re-fogs to black and only re-reveals where a
    // minion currently stands. (A normal new keeper has empty knownTiles, so this loop is a no-op for it.)
    v.setNeedsMemoryUpdate(true);
    ret->addToMemory(v);
  }
  return ret;
}
...
void PlayerControl::loadBuildingMenu(const ContentFactory* contentFactory, const KeeperCreatureInfo& keeperCreatureInfo) {
  for (auto& group : keeperCreatureInfo.buildingGroups)
    for (auto& info : contentFactory->buildInfo.at(group))
      // developerOnly entries are simply never added for an ordinary account. Filtering HERE rather than when
      // the menu is drawn matters: the UI indexes into buildInfo by position (handleSelection), so hiding
      // entries at draw time would shift every index after them and select the wrong thing.
      if (!info.developerOnly || rarIsDeveloper())
        buildInfo.push_back(info);
...
// The keeper's tech tree: the full one, minus anything this keeper can't research at all.
static Technology getKeeperTechnology(const ContentFactory* contentFactory,
...
// Flatten the keeper's workshop groups into one recipe list per workshop type, dropping recipes gated
// behind tech this keeper can never reach.
static WorkshopArray mergeWorkshopGroups(const ContentFactory* contentFactory,
    const KeeperCreatureInfo& keeperCreatureInfo, const Technology& technology) {
...
void PlayerControl::loadImmigrationAndWorkshops(ContentFactory* contentFactory,
    const KeeperCreatureInfo& keeperCreatureInfo) {
  Technology technology = getKeeperTechnology(contentFactory, keeperCreatureInfo);
  collective->setWorkshops(make_unique<Workshops>(
      mergeWorkshopGroups(contentFactory, keeperCreatureInfo, technology), contentFactory));
...
void PlayerControl::reloadBuildMenu(const ContentFactory* contentFactory, const KeeperCreatureInfo& keeperCreatureInfo) {
  // --reload_data: the build menu is baked into `buildInfo` at keeper creation, so re-read it from the fresh
  // factory. Immigration is deliberately left alone -- it holds runtime state we'd lose. Workshops are
  // handled separately by reloadWorkshops(), which can refresh them without dropping the queues.
  buildInfo.clear();
  loadBuildingMenu(contentFactory, keeperCreatureInfo);
}

void PlayerControl::reloadWorkshops(ContentFactory* contentFactory, const KeeperCreatureInfo& keeperCreatureInfo) {
  // --reload_data: a workshop's recipe list is built at spawnKeeper and then serialized into the save
  // (Workshops::Type::options), so an edited workshops_menu.txt otherwise only affects a NEW game. Refresh
  // the options in place; queued items keep their progress, payment and attached runes.
  collective->getWorkshops().refreshOptions(
      mergeWorkshopGroups(contentFactory, keeperCreatureInfo,
          getKeeperTechnology(contentFactory, keeperCreatureInfo)),
      contentFactory);
  for (auto& workshop : collective->getWorkshops().types)
    for (auto& option : workshop.second.getOptions())
      usedResources.insert(option.cost.id);
...
// RAR live PvP: the team LEADER is the only creature the player actually controls -- commandTeam() pushes a
// controller on the leader alone and the rest of the squad follows on AI. So when the leader is killed by the
// defender's authoritative simulation there is nobody controlled left, which read as "defeated": the battle
// ended and the whole surviving squad was sent home, even though the defender was still killing them there.
// Vanilla's own answer (onControlledKilled) is a modal "choose a new leader" dialog -- unusable here, since a
// dialog stops this player's clock while the other keeps playing, which is precisely how the two games drift.
// Hand leadership to a living team member silently instead. Returns false when nobody is left = real defeat.
bool PlayerControl::rarPassLeadership(Creature* victim) {
  auto teamId = getCurrentTeam();
  if (!teamId)
    return false;
  for (Creature* c : getTeams().getMembers(*teamId))
    if (c != victim && !c->isDead()) {
      getTeams().setLeader(*teamId, c);
      if (!c->isPlayer())
        c->pushController(createMinionController(c));
      if (victim->isPlayer())
        victim->popController();   // popped BEFORE it dies, so vanilla's dialog path is never entered
      return true;
    }
  return false;
}
...
void PlayerControl::presentAndClearBattleSummary() {
  ...
  // "Minions killed/captured" must list only YOUR OWN side. When invading an enemy keeper, defenders your
  // team knocks out become prisoners in your collective, so if they die they get recorded here via
  // onMemberKilledOrStunned -- showing slain enemies as your losses. Drop any enemy-tribe creature so only
  // your own minions remain. (enemiesKilled/Captured stay as-is -- those SHOULD be the enemy.)
  auto ownSideOnly = [&](const vector<Creature*>& v) {
    auto pc = getGame()->getPlayerCollective();
    return v.filter([&](Creature* c) { return !pc || !pc->getTribe()->isEnemy(c); });
  };
  auto minionsKilled = ownSideOnly(battleSummary.minionsKilled);
  auto minionsCaptured = ownSideOnly(battleSummary.minionsCaptured);
  if (!minionsKilled.empty())
    data.elems["minions_killed"] = getCreatureList(minionsKilled);
  if (!minionsCaptured.empty())
    data.elems["minions_captured"] = getCreatureList(minionsCaptured);
...
vector<PItem> PlayerControl::retrievePillageItems(Collective* col, vector<Item*> items) {
  ...
  if (!ret.empty()) {
    ...
    // RAR: the loot we just took lives in this villain's downloaded model, whose aftermath was already
    // uploaded WITH that loot. Re-arm its writeback so the emptied state gets re-uploaded -- otherwise the
    // server keeps the original loot and the site is pillageable again on every revisit / relog (and
    // re-taking an item id still sitting in our storage crashes on load). Base-model tiles aren't villains,
    // so this is a no-op for them.
    if (rarEnabled())
      if (auto model = col->getModel())
        getGame()->rearmVillainWriteback(model->position);
  }
  return ret;
}
...
void PlayerControl::refreshGameInfo(GameInfo& gameInfo) const {
  ...
  // RAR: world-map villains have no local Collective (their model is only downloaded when the player travels
  // there), so the loop above can never list them and their attack triggers would be invisible. Add them from
  // the campaign + the wave state Game::considerVillainWaves keeps, so the panel still shows "Triggered" and
  // the "triggered by ..." tooltip. No `action` is set -- pillage/trade need a real Collective.
  if (rarEnabled()) {
    auto& campaign = getGame()->getCampaign();
    // A villain whose model the player has already visited HAS a live collective, so the loop above already
    // listed it -- from the COLLECTIVE's own villain type, which is still whatever the enemy definition says
    // (e.g. ALLY, drawn green) rather than the per-keeper type reconcileVillains worked out for this map.
    // Adding ours too gave two rows for one villain, one of them green "ally". Skip those tiles.
    set<Vec2> alreadyListed;
    for (auto& col : getKnownVillains())
      if (col->getName() && col->isDiscoverable())
        if (auto model = col->getModel())
    ...
    for (const Vec2 pos : toReport) {
      ...
      // map dwelling sprite if the definition somehow has neither.
      info.viewId = ViewIdList{villain->dwellingId};
      if (auto enemyIt = getGame()->getContentFactory()->enemies.find(villain->enemyId);
          enemyIt != getGame()->getContentFactory()->enemies.end()) {
        auto& inhabitants = enemyIt->second.settlement.inhabitants;
        auto& creatureFactory = getGame()->getContentFactory()->getCreatures();
        if (!inhabitants.leader.empty())
      ...
      info.access = VillageInfo::Village::LOCATION;
      info.isConquered = false;
      // Red "attacking" ONLY while its wave is actually on our map and still alive -- not while it is merely
    ...
    if (auto furniture = position.getFurniture(FurnitureLayer::MIDDLE)) {
      ...
      if (auto& obj = furniture->getViewObject()) {
        if (collective->usesEfficiency(furniture)) {
          // Guard like the sibling getObject calls below (2421) and above (2384): if this territory tile isn't
          // currently visible and its memory index lacks the furniture's layer object, getObject() would CHECK-
          // crash ("No object on layer 1"). Seen e.g. scrolling to a z-level whose territory tiles aren't in view.
          if (index.hasObject(obj->layer()))
...
void PlayerControl::rarSetViewPos(Position p) {
  setScrollPos(p);
}

long long PlayerControl::rarGetViewLevelId() const {
  return getCurrentLevel()->getUniqueId();
}

Level* PlayerControl::rarSwapCurrentLevel(Level* l) {
  auto old = currentLevel;
  currentLevel = l;
  return old;
}
...
optional<TeamId> PlayerControl::rarControlTeam(vector<Creature*> team) {
  // Control the WHOLE squad as one team (controlling only the leader left the rest on AI -- they walked home).
  // Returns a team id ONLY when we created the team, because the caller cancels what it gets back when the
  // battle ends (vanilla auto-cancels single-member teams only). none = the squad already was the player's own
  // team, which is NOT ours to cancel.
  team = team.filter([](Creature* c) { return c && !c->isDead(); });
  if (team.empty())
    return none;
  // The player forms a team to send on the invasion in the first place, so creating a second one with the same
  // members put two identical rows in the Teams panel for the whole battle -- and left a duplicate behind on any
  // battle end that skipped the teardown. Reuse the player's team when it is this same squad.
  for (TeamId existing : getTeams().getContaining(team[0])) {
    auto members = getTeams().getMembers(existing).filter([](Creature* c) { return c && !c->isDead(); });
    if (members.size() != team.size())
      continue;
    bool same = true;
    for (Creature* c : team)
      if (!getTeams().contains(existing, c)) {
        same = false;
        break;
      }
    if (same) {
      commandTeam(existing);
      return none;
    }
  }
  auto id = getTeams().create(team);
  commandTeam(id);
  return id;
}
  ...
  building.visit<void>(
    ...
    [&](const BuildInfoTypes::PlaceItemNew&) {
      ...
      for (auto& elem : factory->items) {
        auto sample = ItemType(elem.first).get(factory);
        vector<TString> details;
        // One line per stat the item ACTUALLY has: getModifierValues() holds only the attrs it defines, so
        // armour contributes a defense line, a weapon a damage line, and a plain rock contributes none --
        // no hardcoded attr list. PLUS_MINUS_ATTR is the same format string Item::getModifiers() uses, so the
        // wording matches the rest of the UI and needs no new translations.
        for (auto& modifier : sample->getModifierValues())
          details.push_back(TSentence("PLUS_MINUS_ATTR", toStringWithSign(modifier.second),
              factory->attrInfo.at(modifier.first).name));
        // Usage effect, victim/attacker effects, equipped effects, granted abilities, upgrade info.
        details.append(sample->getDescription(factory));
        choices.push_back({sample->getViewObject().getViewIdList(), sample->getName(), std::move(details)});
        types.push_back(ItemType(elem.first));
      }
      if (auto idx = getView()->chooseItem(TString("Choose an item to place"_s), choices, TStringId("CANCEL_BUTTON")))
        if (auto num = getView()->getNumber(TStringId("ENTER_AMOUNT"), Range(1, 10000), 1))
          position.dropItems(types[*idx].get(*num, factory));
    },
    ...
    [&](BuildInfoTypes::PlaceMinionNew) {
      // Same as PlaceMinion, but a creature that can dig also gets the WORKER trait so it actually digs +
      // hauls (plain PlaceMinion only sets FIGHTER, so a placed imp just stands around).
      auto& factory = getGame()->getContentFactory()->getCreatures();
      vector<PCreature> allCreatures = factory.getAllCreatures().transform(
          [this, &factory](CreatureId id){ return factory.fromId(id, getTribeId()); });
      if (auto id = getView()->chooseCreature(TStringId("CHOOSE_CREATURE_TO_PLACE"),
        ...
        for (auto& c : allCreatures)
          if (c->getUniqueId() == *id) {
            EnumSet<MinionTrait> traits{MinionTrait::FIGHTER};
            if (c->getAttr(AttrType("DIGGING")) > 0)
              traits.insert(MinionTrait::WORKER);
            collective->addCreature(std::move(c), position, traits);
          }
      }
    },
...
void PlayerControl::checkKeeperDanger() {
  // RAR: an INJECTED foreign keeper (the invasion target) carries its own PlayerControl. It must not run
  // the player-side "keeper in danger -> take control?" prompt -- that would make the invader control (and
  // then, on killing it, trigger the death/message-history flow for) the ENEMY keeper. Only the real player's.
  if (collective != getGame()->getPlayerCollective())
    return;
...
void PlayerControl::update(bool currentlyActive) {
  ...
  // RAR: never auto-recruit while invading another dungeon. The recruit test keys off the COLLECTIVE's tribe
  // (still the invader's normal tribe), so a same-alignment keeper's defenders would otherwise "join your
  // team" instead of fighting -- the invading team's creatures are separately retagged to Invaders for combat.
  bool invading = getGame()->isInvading();
  for (Level* l : currentLevels)
    for (Creature* c : l->getAllCreatures())
      if (!invading && !getCreatures().contains(c) && c->getTribeId() == getTribeId() && canSee(c) && !isEnemy(c) && canAllyJoin(c)) {
...
void PlayerControl::scrubInvadedModelRefs(const Model* invaded) {
  // Message logs hold a location Position -> raw Level*; drop just the link for invaded-model messages
  // (keep the text). The invaded levels are still alive here so getModel() is reliable.
  auto scrubMsgs = [&](vector<PlayerMessage>& v) {
    for (auto& msg : v)
      if (auto p = msg.getPosition())
        if (p->getModel() == invaded)
          msg.clearPosition();
  };
  scrubMsgs(messages);
  scrubMsgs(messageHistory);
  if (controlModeMessages) {
    scrubMsgs(controlModeMessages->current);
    scrubMsgs(controlModeMessages->history);
  }
  // Transient combat/notification state holding raw Creature*/Collective* into the invaded model
  // (BattleSummary's 4 creature lists -- incl. enemiesKilled from slain defenders -- pending attacks,
  // and stun-capture pairs). Matching-by-model is unreliable for dead creatures, and it's all transient
  // UI state, so reset it wholesale.
  battleSummary = BattleSummary{};
  newAttacks.clear();
  notifiedAttacks.clear();
  stunnedCreatures.clear();
}

void PlayerControl::rarClearStaleRefs() {
  // The visibility map outlives the battlefield: it keeps per-creature tile lists pointing into the model that
  // is about to be freed. Nothing here may dereference a Position, so it is cleared wholesale.
  visibilityMap->rarClearAll();
  auto clearMsgs = [](auto& v) {
    for (auto& msg : v)
      msg.clearPosition();   // never read the Position: it may point into an already-freed model
  };
  clearMsgs(messages);
  clearMsgs(messageHistory);
  if (controlModeMessages) {
    clearMsgs(controlModeMessages->current);
    clearMsgs(controlModeMessages->history);
  }
  battleSummary = BattleSummary{};
  newAttacks.clear();
  notifiedAttacks.clear();
...
void PlayerControl::considerNewAttacks() {
  for (auto attack : copyOf(newAttacks))
    if (isConsideredAttacking(attack)) {
      ...
      // A pay-ransom prompt is a blocking question about THIS attack, so it still scrolls there -- the player
      // is being asked to decide and needs to see what he's deciding about.
      const bool ransomPrompt = attack.getRansom()
          && collective->hasResource({ResourceId("GOLD"), *attack.getRansom()})
          && !getGame()->getOptions()->getBoolValue(OptionId::NEVER_PAY_RANSOM);
      // The quiet notice deliberately does NOT scroll. Yanking the camera across the map is exactly the kind of
      // interruption DISABLE_ATTACK_WARNING exists to stop -- suppressing only the popup while still stealing
      // the view left the option half-doing its job. The message carries the position, so the player can still
      // click it to go there when HE wants to.
      const bool quietWarning = !ransomPrompt
          && getGame()->getOptions()->getBoolValue(OptionId::DISABLE_ATTACK_WARNING);
      if (!quietWarning) {
        setScrollPos(attack.getCreatures()[0]->getPosition().plus(Vec2(0, 5)));
        getView()->refreshView();
      }
      // "Never pay ransom" option: skip the pay-ransom prompt entirely and fall through to the normal
      // under-attack notice, so the attack just proceeds without bothering the player.
      if (ransomPrompt) {
      ...
      } else if (quietWarning)
        // Non-blocking notice: push to the message log at HIGH priority (only CRITICAL stops the clock), so the
        // player is told "you are under attack by xyz" without the game-pausing popup window -- and, per the
        // scroll guard above, without the camera jumping either.
        addMessage(PlayerMessage(TSentence("YOU_ARE_UNDER_ATTACK_BY", attack.getAttackerName()),
            MessagePriority::HIGH).setPosition(attack.getCreatures()[0]->getPosition()));
      else
...
void PlayerControl::onConquered(Creature* victim, Creature* killer) {
  // RAR invasion: an INJECTED foreign keeper (the invasion target) carries its own PlayerControl.
  // Conquering it must NOT end the INVADER's game -- only the game's actual player collective does.
  if (collective != getGame()->getPlayerCollective())
    return;
  if (!victim->isPlayer()) {
...
CollectiveControl::DuelAnswer PlayerControl::acceptDuel(Creature* attacker) {
  // RAR: an injected foreign keeper's collective must not prompt the invader to accept a duel (which would
  // hand the invader control of the enemy keeper). Only the real player's collective answers duels.
  if (collective != getGame()->getPlayerCollective())
