...
void Collective::serialize(Archive& ar, const unsigned int version) {
  ...
  // Added in version 1. Gated, so a save written before this field existed still loads (the counter simply
  // starts at 0 for those colonies) -- Collective::serialize is one flat list, so an ungated field would have
  // broken every existing save.
  if (version >= 1)
    ar(minionsLost);
}
  ...
  if (Random.roll(5)) {
    for (Position pos : territory->getAll())
      if (!isDelayed(pos) && pos.canEnterEmpty(MovementTrait::WALK) && !pos.getItems().empty())
        fetchItems(pos);
    // getPositions returns an unordered PositionSet -> iterate in deterministic order so item-fetch task creation
    // is lockstep-safe (order affects task ids + RNG consumption).
    for (Position pos : positionsInDeterministicOrder(zones->getPositions(ZoneId::FETCH_ITEMS)))
      if (!isDelayed(pos) && pos.canEnterEmpty(MovementTrait::WALK))
        fetchItems(pos);
    for (Position pos : positionsInDeterministicOrder(zones->getPositions(ZoneId::PERMANENT_FETCH_ITEMS)))
      if (!isDelayed(pos) && pos.canEnterEmpty(MovementTrait::WALK))
        fetchItems(pos);
  }
...
void Collective::onMinionKilled(Creature* victim, Creature* killer) {
  ...
  if (!hasTrait(victim, MinionTrait::FARM_ANIMAL) && !hasTrait(victim, MinionTrait::SUMMONED))
    ++minionsLost;   // a real member of the colony died -- same exclusions the morale code below uses
  if (!hasTrait(victim, MinionTrait::FARM_ANIMAL) && !hasTrait(victim, MinionTrait::SUMMONED)) {
...
void Collective::claimWallFurniture(Position pos) {
  ...
  for (auto layer : {FurnitureLayer::FLOOR, FurnitureLayer::MIDDLE, FurnitureLayer::CEILING})
    if (auto furniture = pos.modFurniture(layer))
      if (!furniture->forgetAfterBuilding() && !furniture->isStairs()) {
  ...
  control->onClaimedSquare(pos);   // this is what fires onConstructed -> visibilityMap->updateEyeball()
}
...
void Collective::limitKnownTilesToOwnModel() {
...
int Collective::getMinionsLost() const {
  return minionsLost;
}
...
void Collective::onExternalEnemyKilled(const TString& name) {
  // CRITICAL is precisely what pauses the game -- PlayerControl::addMessage() calls getView()->stopClock() on
  // it and nothing else. So with the under-attack warning disabled, downgrade to HIGH: the player is still
  // told he resisted, in the message log, but the wave ending doesn't stop the clock on him. Same trade the
  // attack notice itself makes -- keep the information, drop the interruption.
  auto priority = MessagePriority::CRITICAL;
  if (auto game = getGame())
    if (game->getOptions()->getBoolValue(OptionId::DISABLE_ATTACK_WARNING))
      priority = MessagePriority::HIGH;
  control->addMessage(PlayerMessage(TSentence("YOU_RESISTED_THE_ATTACK_OF", name), priority));
