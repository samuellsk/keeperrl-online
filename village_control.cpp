...
void VillageControl::onEvent(const GameEvent& event) {
  ...
  event.visit<void>(
      ...
      [&](const CreatureKilled& info) {
        // A slain creature drops its gear + corpse via Position::dropItem (which does NOT fire an
        // ItemsDropped event), so without this the pillage cache would stay stale after a conquest and
        // pillage wouldn't appear until some later item event happened to clear it. Recompute it now.
        canPillageCache = none;
      },
