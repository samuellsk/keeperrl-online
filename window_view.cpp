  ...
  mapGui.reset(new MapGui({
      ...
      [this] (Vec2 pos) {
        if (!guiBuilder.getActiveButton()) {
          ...
          rarLastClickMs = (int) clock->getRealMillis().count(); // diagnostic: when the click entered the queue
          rarActionCalls = 0;                                    // diagnostic: count getAction() calls from here
...
void WindowView::rebuildGui() {
  ...
  auto getMovement = [this](int x, int y) {
    // RAR live PvP: keyboard movement is disabled outright. Two reasons, and the second is the damaging one:
    // the defender's simulation owns every position, so a locally-walked step is corrected back anyway; and a
    // QUEUED input makes the game loop advance a FULL turn per frame (see rarHasPendingInput in playGame) instead
    // of the real-time fraction -- so holding a direction key ran this player's clock several times faster than
    // the other player's, which is exactly how the two games drift apart. Movement is ordered by clicking.
    if (rarLivePvpInput)
      return;
...
void WindowView::rarSetInteractiveSpectator(bool b) {
  rarInteractiveSpectator = b;
}

int WindowView::rarConsumeSpectatorKey() {
  int k = rarSpectatorKey;
  rarSpectatorKey = 0;
  return k;
}

bool WindowView::rarHasPendingInput() {
  return !inputQueue.isEmpty();
}

int WindowView::rarConsumeClickMs() {
  int r = rarLastClickMs;
  rarLastClickMs = 0;
  return r;
}

int WindowView::rarGetActionCalls() {
  return rarActionCalls;
}

void WindowView::rarPumpEvents() {
  // The cheap half of refreshView(): read input, draw nothing. Same guards as refreshView.
  CHECK(currentThreadId() == renderThreadId);
  if (gameReady || !blockingElems.empty())
    processEvents();
}

void WindowView::rarSetSpeedLocked(bool b) {
  guiBuilder.rarSetSpeedLocked(b);
}

void WindowView::rarSetLivePvpInput(bool b) {
  rarLivePvpInput = b;
  if (b)
    lockKeyboard = false; // don't stay locked from the last turn-based turn
}
...
optional<Vec2> WindowView::chooseSite(const TString& message, const Campaign& campaign, Vec2 current,
  ...
  SyncQueue<optional<Vec2>> returnQueue;
  return getBlockingGui(returnQueue, guiBuilder.drawChooseSiteMenu(returnQueue, message, campaign, current, invadeable));
}

void WindowView::presentWorldmap(const Campaign& campaign, Vec2 current, bool preview) {
  Semaphore sem;
  return getBlockingGui(sem, guiBuilder.drawWorldmap(sem, campaign, current, preview));
}
...
optional<int> WindowView::chooseItem(const TString& title, const vector<ItemChoiceInfo>& items,
    const TString& cancelText) {
  SyncQueue<optional<int>> returnQueue;
  return getBlockingGui(returnQueue, guiBuilder.drawChooseItemMenu(returnQueue, title, items, cancelText));
}

View::LayoutPreviewAction WindowView::previewLayoutMenu(const Campaign* campaign, const LayoutPreviewInfo& info) {
  SyncQueue<LayoutPreviewAction> returnQueue;
  return getBlockingGui(returnQueue, guiBuilder.drawLayoutPreviewMenu(returnQueue, campaign, info));
}
...
void WindowView::processEvents() {
  ...
  while (renderer.pollEvent(event)) {
    considerResizeEvent(event);
    if (gameInfo.infoType == GameInfo::InfoType::SPECTATOR && rarInteractiveSpectator) {
      // RAR live PvP: interactive spectator (the invader's battlefield view). ESC exits; everything else
      // propagates so the map scrolls and responds. (Plain SPECTATOR below exits on ANY key -- intro splash.)
      if (event.type == SDL::SDL_KEYDOWN) {
        if (event.key.keysym.sym == SDL::SDLK_ESCAPE) {
          inputQueue.push(UserInput(UserInputId::EXIT));
          return;
        }
        rarSpectatorKey = event.key.keysym.sym; // capture command keys (stop / don't-chase) for the observer loop
      }
      propagateEvent(event, clickableGuiElems);
    }
    else if (gameInfo.infoType == GameInfo::InfoType::SPECTATOR)
      switch (event.type) {
      ...
      // RAR live PvP: don't lock the keyboard between turns. Normally one keypress is accepted per turn (turn-based),
      // but in a real-time battle the player presses while the creature is still acting; those presses must QUEUE,
      // not be dropped. (Pairs with skipping flushEvents below.)
      if (event.type == SDL::SDL_KEYDOWN && renderDialog.empty() && blockingElems.empty() && !rarLivePvpInput) {
        if (lockKeyboard)
          return;
        lockKeyboard = true;
      }
    ...
    switch (event.type) {
      case SDL::SDL_KEYDOWN:
        // Turn-based control flushes queued keys so a held/mashed key doesn't run several turns. In live PvP the
        // clock keeps running, so keep them: each press becomes an order executed on the creature's next turn.
        if (gameInfo.infoType == GameInfo::InfoType::PLAYER && !rarLivePvpInput)
...
UserInput WindowView::getAction() {
  ++rarActionCalls; // diagnostic: how many times input was requested since the last map click
  lockKeyboard = false;
  if (refreshInput) {
    refreshInput = false;
