...
class PlayerInfo {
  ...
  // RAR live PvP ONLY: present => show the Auto Fight button, and its current state. Unset in every normal
  // game, which is what keeps the button out of the ordinary UI.
  optional<bool> HASH(rarAutoFight);
  // True while a live PvP battle is running. Used to hide the Leader/Full control TOGGLE, which is meaningless
  // when the squad is commanded by clicking. It must be a SEPARATE flag: leaving controlMode itself unset makes
  // drawRightPlayerInfo() bail out at its first line and drop the whole team panel -- including Exit Control.
  bool HASH(rarLivePvp) = false;
