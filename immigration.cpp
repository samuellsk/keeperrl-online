...
static vector<Position> getSpawnPositions(const Collective* collective, const ImmigrantInfo& info) {
  ...
  return info.getSpawnLocation().visit<vector<Position>>(
    [&] (FurnitureType type) {
      // deterministic order (getBuiltPositions is unordered) so immigrant spawn placement is lockstep-safe
      return positionsInDeterministicOrder(collective->getConstructions().getBuiltPositions(type));
    },
    [&] (OutsideTerritory) {
