...
PrettyInputArchive::PrettyInputArchive(const vector<string>& inputs, const vector<string>& filenames, KeyVerifier* v)
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    // SEPARATOR between files. Vanilla and every active mod's same-named config are concatenated into one
    // literal; appending them back to back means a file that does not end with a newline glues its last token
    // to the next file's first one. A mod ending in `End` followed by one starting with `Def` produced the
    // token "EndDef", which parseDefs then failed to recognise as the End of a macro -- so it swallowed the
    // next mod's definition and reported a baffling error in a file that was perfectly fine. Costs nothing:
    // whitespace between top-level entries is insignificant, and the wrapper below already adds its own.
    if (i > 0)
      allInput.push_back(StreamChar{{}, '\n'});
