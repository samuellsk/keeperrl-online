...
#include "task.h" // SummonInvasion: Task::attackCreatures -> the kill-the-keeper goal
#include "tribe.h"
...
static TString getName(const Effects::SummonInvasion& e, const ContentFactory* f) {
  return TSentence("SUMMON_HOSTILE_EFFECT_NAME", f->getCreatures().getName(e.creature));
}

static bool isOffensive(const Effects::SummonInvasion&) {
  return true;
}

static TString getDescription(const Effects::SummonInvasion& e, const ContentFactory* f) {
  if (e.count.getEnd() > 2) {
    if (e.count.getLength() == 1)
      return TSentence("SUMMON_HOSTILE_MULTIPLE_EFFECT_DESCRIPTION", f->getCreatures().getNamePlural(e.creature),
          TString(e.count.getStart()));
...
static bool apply(const Effects::SummonInvasion& e, Position pos, Creature*) {
  ...
  if (!target)
    return false;
  // The whole point of this effect: the wave hunts the keeper rather than whatever wanders past. Same target
  // and same fallback as the KillLeader external-enemy behaviour.
  auto attackTask = [&] () -> PTask {
    if (!target->getLeaders().empty())
      return Task::attackCreatures(target->getLeaders());
    if (!target->getCreatures(MinionTrait::FIGHTER).empty())
      return Task::attackCreatures(target->getCreatures(MinionTrait::FIGHTER));
    return nullptr;
  }();
  if (!attackTask)
    return false;
  ...
  auto creatureList = CreatureList(Random.get(e.count), e.creature);
  // One task shared by the whole wave, via the same path the endless waves use: singleTask() hands its task to
  // the first creature that asks, so the AI factory MUST be copied per creature -- generate() takes it by value
  // and Monster::getFactory() copies it again into each controller, which is what makes the sharing safe.
  auto creatures = creatureList.generate(Random, &creatureFactory, TribeId::getMonster(),
      MonsterAIFactory::singleTask(std::move(attackTask)));
  auto ret = Effect::summonCreatures(pos, std::move(creatures), 1_visible);
  if (ret.empty())
    return false;
  for (auto c : ret) {
    c->setCombatExperience(pos.getModelDifficulty());
...
static bool apply(const Effects::RevealAndClaimTile& a, Position pos, Creature* attacker) {
  if (auto col = pos.getGame()->getPlayerCollective()) {
    if (col->getTerritory().contains(pos))
      return false;
    // Reveal first: addKnownTile marks the tile known AND sets needsRenderAndMemoryUpdate, which is what lifts
    // the fog. It is also what makes canClaimSquare() accept the tile at all.
    col->addKnownTile(pos);
    if (col->canClaimSquare(pos)) {
      col->claimSquare(pos);
      return true;
    }
    return false;   // still refused (a wall, or not covered) -- but it is revealed now
  }
  return false;
}

static TString getName(const Effects::RevealAndClaimTile&, const ContentFactory*) {
  return TStringId("CLAIM_TILE_EFFECT_NAME");
}

static TString getDescription(const Effects::RevealAndClaimTile&, const ContentFactory*) {
  return TStringId("CLAIM_TILE_EFFECT_DESCRIPTION");
}

static bool apply(const Effects::ClaimWalls& a, Position pos, Creature* attacker) {
  auto col = pos.getGame()->getPlayerCollective();
  if (!col)
    return false;
  // Only the listed types, on any layer. Deliberately NOT "every wall here": claiming rock/hard rock by
  // accident would hand the player the whole mountain.
  bool found = false;
  for (auto layer : {FurnitureLayer::FLOOR, FurnitureLayer::MIDDLE, FurnitureLayer::CEILING})
    if (auto furniture = pos.getFurniture(layer))
      if (a.types.contains(furniture->getType())) {
        found = true;
        break;
      }
  if (!found)
    return false;
  col->claimWallFurniture(pos);
  return true;
}

static TString getName(const Effects::ClaimWalls&, const ContentFactory*) {
  return TStringId("CLAIM_TILE_EFFECT_NAME");
}

static TString getDescription(const Effects::ClaimWalls&, const ContentFactory*) {
  return TStringId("CLAIM_TILE_EFFECT_DESCRIPTION");
}
...
static EffectAIIntent shouldAIApply(const Effects::Name& e, const Creature* caster, Position pos) {
  PROFILE_BLOCK("Effects::Name::shouldAIApply");
  return e.effect->shouldAIApply(caster, pos);
}
...
// Content-file syntax only. `summoned` is live state and is deliberately absent -- it is never authored, it is
// filled in at runtime and saved with the furniture instance.
template <>
void Effects::SummonInvasion::serialize(PrettyInputArchive& ar, const unsigned int version) {
  ar(creature);
