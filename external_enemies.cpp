...
ExternalEnemies::ExternalEnemies(RandomGen& random, CreatureFactory* factory, vector<ExternalEnemy> enemies, ExternalEnemiesType type) {
  ...
  // How many turns before the first wave. FROM_START keeps vanilla timing; the others push the WHOLE
  // progression back, so waves still ramp from easy to hard -- they just begin later.
  const int startDelay = [&] {
    switch (type) {
      case ExternalEnemiesType::AFTER_25K: return 25000;
      case ExternalEnemiesType::AFTER_50K: return 50000;
      case ExternalEnemiesType::AFTER_100K: return 100000;
      default: return 0;
    }
  }();
  // Always start the clock. AFTER_WINNING used to leave this unset and wait for Game::gameWon(), which never
  // happens in this game -- the delay is applied per wave below instead.
  startTime = 0_local;
  for (int i : Range(500)) {
    ...
    for (int index : random.permutation(indexes)) {
      ...
      if (attackRange.contains(attackTime) && enemy.maxOccurences > 0) {
        ...
        waves.push_back(EnemyEvent{
            enemy,
            LocalTime(attackTime + startDelay),   // same progression, pushed back by the chosen delay
