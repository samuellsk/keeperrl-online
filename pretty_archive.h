...
class PrettyInputArchive {
  public:
    ...
    // A variant with a DEFAULT_ELEM falls back to it by seeking back and re-parsing the same text. When the
    // default elem itself contains the variant (Effect's default is Chain, i.e. vector<Effect>), an unknown
    // type name makes that fallback re-read the SAME name forever -- the parse blows the stack instead of
    // saying "X is not part of variant", so a mod with one typo'd effect crashes with no message at all.
    // Legitimate nesting always advances the stream, so a bookmark that is already being parsed as a default
    // elem means we are in that loop.
    vector<long> defaultElemStack;
    bool enterDefaultElem(long b) {
      if (defaultElemStack.contains(b))
        return false;
      defaultElemStack.push_back(b);
      return true;
    }
    void leaveDefaultElem() {
      defaultElemStack.pop_back();
    }
