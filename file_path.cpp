...
#ifdef WINDOWS
  struct _stat buf;
  // MUST check _stat's return: on failure (file missing) buf is left uninitialized, and reading buf.st_mode
  // then returns garbage -- often the STALE value from a previous _stat call at the same stack slot, so a
  // non-existent file wrongly reports as existing. That makes readObject() add a missing per-mod config file
  // (e.g. a layout-only mod's absent keeper_creatures.txt) to its path list, which then fails to open. The
  // POSIX branch below already guards this; match it.
